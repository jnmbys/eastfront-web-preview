...
function cityDistricts(p) {
    const centers = p.cityCells.map(c => ({ x: c.polygon.reduce((s, v) => s + v.x, 0) / 6, y: c.polygon.reduce((s, v) => s + v.y, 0) / 6 }));
    const unseen = new Set(centers), districts = [];
    for (const start of centers) {
        if (!unseen.delete(start))
            continue;
        const group = [start];
        for (let i = 0; i < group.length; i++)
            for (const c of unseen) {
                if (Math.hypot(c.x - group[i].x, c.y - group[i].y) < VS2_WORLD_H * 1.05) {
                    unseen.delete(c);
                    group.push(c);
                }
            }
        ...
        for (let iy = Math.floor(Math.min(...ys) / step); iy <= Math.ceil(Math.max(...ys) / step); iy++) {
            ...
            for (let ix = Math.floor(Math.min(...xs) / step); ix <= Math.ceil(Math.max(...xs) / step); ix++) {
                ...
                if (seen.has(key))
                    continue;
                seen.add(key);
                const random = (tag) => vs2VisualValue(seed, `F1-urban:${tag}`, ix, iy), r = random('density');
                ...
                if (r < 0.06 + (1 - density) * 0.22)
                    continue;
                const nearest = transport.reduce((best, e) => !best || vs2SegmentDistance({ x, y }, e.a, e.b) < vs2SegmentDistance({ x, y }, best.a, best.b) ? e : best, undefined);
                ...
                if (!projection.canPlaceCity({ x, y }, width + 2, height + 2))
                    continue;
    ...
    for (const d of districts) {
        ...
        coreSearch: for (const [scaleWidth, scaleHeight] of [[2.1, 2.35], [1.65, 1.85]]) {
            ...
            for (const focus of local) {
                ...
                if (!projection.canPlaceCity(focus, width + 2, height + 2))
                    continue;
                const conflicts = blocks.filter(b => b !== focus && Math.abs(b.x - focus.x) < (b.width + width) / 2 + 1 && Math.abs(b.y - focus.y) < (b.height + height) / 2 + 1);
                if (conflicts.length > 3 || conflicts.some(b => b.landmark))
                    continue;
                const remaining = blocks.filter(b => !conflicts.includes(b));
                if (cellCenters.some(c => remaining.filter(b => Math.hypot(b.x - c.x, b.y - c.y) < 35).length < 5))
                    continue;
        ...
        if (!consolidated && local[0])
            blocks[blocks.indexOf(local[0])] = { ...local[0], landmark: true };
    }
...
function* planVS2CityCourtsWork(p, blocks) {
    ...
    for (let i = 0; i < blocks.length; i++) {
        ...
        for (const b of blocks.slice(i + 1)) {
            ...
            if (p.canPlaceCity(center, Math.abs(a.x - b.x) + width + 2, Math.abs(a.y - b.y) + width + 2))
                courts.push({ a, b, width });
        }
    }
    return courts;
}
export function paintVS2CityCourts(ctx, courts) {
    ctx.save();
    try {
        ctx.lineCap = 'round';
        for (const { a, b, width } of courts) {
            ctx.globalAlpha = 0.56;
            ctx.strokeStyle = '#666149';
...
export function paintVS2CityMassing(ctx, blocks) {
    ctx.save();
    try {
        for (const b of blocks) {
            ...
            ctx.fillStyle = '#c5ab7d';
            ctx.fillRect(x - 0.7, y - 0.7, w + 1.4, h + 1.4);
            ctx.globalAlpha = 0.88;
            ctx.fillStyle = '#393e34';
            ctx.fillRect(x + 0.5, y + 0.5, w, h);
            ctx.globalAlpha = 1;
            ctx.fillStyle = b.density > 0.45 ? '#eee0bb' : '#ded1ae';
            ...
            ctx.moveTo(x, y + h * 0.48);
            ctx.lineTo(x + w, y + h * 0.48);
            ctx.lineTo(x + w * 0.85, y + h * 0.87);
            ctx.lineTo(x + w * 0.15, y + h * 0.87);
            ctx.closePath();
            ctx.fill();
            ctx.fillStyle = '#e6d2a9';
            ctx.fillRect(x + w * 0.17, y + h * 0.45, w * 0.66, 0.28);
            if (b.industrial) {
                ctx.fillStyle = '#76604a';
                ctx.fillRect(x + w * 0.68, y + h * 0.08, 0.85, h * 0.55);
            }
            if (b.landmark) {
                ctx.fillStyle = '#354f49';
                ctx.beginPath();
                ctx.moveTo(-w * 0.16, h * 0.12);
                ctx.lineTo(-w * 0.16, -h * 0.23);
                ctx.lineTo(0, -h * 0.48);
                ctx.lineTo(w * 0.16, -h * 0.23);
                ...
                ctx.fillRect(-w * 0.1, -h * 0.04, w * 0.2, h * 0.28);
            }
            ctx.restore();
        }
    }
