...
function syncFogSurface() {const __end=__trace.begin("syncFogSurface");try{
...
function startNewGame() {const __end=__trace.begin("startNewGame");try{ terrainPipeline?.resume(); terrainPipeline?.continueAll(); deploymentTouch = createDeploymentTouch(); if (!productionMap || !cachedTerrainSurface) {
    appStatus = 'FATAL';
    fatalMessage = msg('game.noMap');
    render();
    return;
} session = createFreshProductionSession(productionMap); presentation = createPresentationState(developerUi && query.get('debug') === '1', window.matchMedia('(max-width: 1100px)').matches); presentation.rendererMode = 'production'; presentation.productionAssetSet = 'p5'; appStatus = 'PLAYING'; fatalMessage = ''; render(); }finally{__end();}}
...
function mountCachedTerrainSurface() {const __end=__trace.begin("mountCachedTerrainSurface");try{
...
function refreshDynamicView() {const __end=__trace.begin("refreshDynamicView");try{
...
function render() {const __end=__trace.begin(`render:${appStatus}`);try{
...
async function enterNetworkMatch(client) {const __end=__trace.begin("enterNetworkMatch");try{
...
async function boot(networkModel) {const __end=__trace.begin("boot");try{
    if (terrainBoot)
        return terrainBoot;
...
async function prepareTerrain(networkModel) {const __end=__trace.begin("prepareTerrain");try{
    appStatus = 'LOADING';
    render();
    ...
    try {
        ...
        const pipeline = new ProgressiveTerrain((lod, control) => __trace.async(`surface:${lod}`,()=>buildCachedTerrainSurface(staticModel, TERRAIN_VISUAL_SEED, 'p5', lod, vs2.worldBaseFor(control))), lod => {
