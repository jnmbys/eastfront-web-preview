...
function syncFogSurface() {const __p4end=perf4.begin("main.js:syncFogSurface");try{
...
function paintDeploymentFocus(model) {const __p4end=perf4.begin("main.js:paintDeploymentFocus");try{
    syncFogSurface();
...
function sidePanelMarkup(model, locations) {const __p4end=perf4.begin("main.js:sidePanelMarkup");try{
...
function refreshDynamicView() {const __p4end=perf4.begin("main.js:refreshDynamicView");try{
    ...
    const panelScroll = perf4.sync('layout:read-panelScroll',()=>panel.querySelector('.command-panel-scroll')?.scrollTop ?? 0);
    const rosterScroll = perf4.sync('layout:read-rosterScroll',()=>panel.querySelector('.roster-list')?.scrollTop ?? 0);
    const locationScroll = perf4.sync('layout:read-locationScroll',()=>panel.querySelector('.location-grid')?.scrollTop ?? 0);
    ...
    if (!retainedPanel) {
        ...
        if (scroll)
            perf4.sync('layout:write-panelScroll',()=>{scroll.scrollTop = panelScroll;});
        const roster = panel.querySelector('.roster-list'), locations = panel.querySelector('.location-grid');
        if (roster)
            perf4.sync('layout:write-rosterScroll',()=>{roster.scrollTop = rosterScroll;});
        if (locations)
            perf4.sync('layout:write-locationScroll',()=>{locations.scrollTop = locationScroll;});
    }
    bindDynamic(model);
...
function render() {const __p4end=perf4.begin("main.js:render");try{
...
function bindDynamic(model) {const __p4end=perf4.begin("main.js:bindDynamic");try{
