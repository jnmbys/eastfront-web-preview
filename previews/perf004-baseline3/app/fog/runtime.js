...
function encode(raster) {const __p4end=perf4.begin("fog/runtime.js:encode");try{
    ...
    perf4.sync('fog:upload',()=>ctx.putImageData(data, 0, 0));
    return perf4.sync('fog:encodePNG',()=>canvas.toDataURL('image/png'));
}finally{__p4end();}}
...
export class FogRuntime {
    ...
    sync(svg, view, selectedUnitId = null, instant = false) {const __p4end=perf4.begin("fog/runtime.js:FogRuntime.sync");try{
        if (!svg || !view) {
            this.clear();
            return;
        }
