...
export function bindLanguageControl(root, repaint) {
    root.querySelectorAll('[data-language-select]').forEach(control => control.addEventListener('change', () => {
        ...
        root.querySelector('[data-language-select]')?.focus({ preventScroll: true });
    }));
}
