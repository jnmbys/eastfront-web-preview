...
export function deriveBrowserRenderModel(session, presentation) {const __p4end=perf4.begin("core-adapter/browserProjection.js:deriveBrowserRenderModel");try{
    ...
    const counters = visibleUnits.map(unit => ({ id: unit.id, side: unit.side, type: unit.type, step: unit.step, stats: { ...unit.stats }, supplyState: unit.supplyState, controllerId: 'friendly' in unit ? unit.friendly.controllerId : '', hex: { ...unit.hex }, selected: unit.id === presentation.selectedUnitId, entrenched: unit.entrenched }));
    ...
    if (!readOnly && (session.state.phase === 'GERMAN_COMBAT' || session.state.phase === 'SOVIET_COMBAT') && (!session.state.pendingDecision || playerView.pendingDecision)) {
        ...
        const draftPrimary = primaryAttackerId(presentation);
        const primary = draftPrimary && attackerUnitIds.includes(draftPrimary) ? draftPrimary : null;
        ...
        for (const id of attackerUnitIds) {
            ...
            for (const h of getNeighbors(u.hex)) {
                const defenders = visibleUnits.filter((d) => d.side !== u.side && d.hex.q === h.q && d.hex.r === h.r);
                if (defenders.length)
        ...
        const publicContext = (context) => context ? { ...structuredClone(context), attackerArtilleryUnitId: context.attackerArtilleryUnitId && visibleIds.has(context.attackerArtilleryUnitId) ? context.attackerArtilleryUnitId : null, defenderArtilleryUnitId: context.defenderArtilleryUnitId && visibleIds.has(context.defenderArtilleryUnitId) ? context.defenderArtilleryUnitId : null } : null;
