import { coreHexKey, getLegalRetreatStepOptions, getMaxLegalRetreatDistance, getNeighbors, } from '../core-adapter/core.js';
import { controllerIdForSide, dispatchGameAction, setActiveViewer } from '../core-adapter/session.js';
import { clearCombatDrafts } from '../state/presentation.js';
...
function accepts(session, action) {
    return session.engine.apply(structuredClone(session.state), action).accepted;
}
export function reactionChoices(session) {
    const p = session.state.pendingDecision;
    if (p?.kind !== 'DEFENDER_REACTION')
        return { artillery: [], hq: [] };
    const base = { type: 'COMBAT_REACTION', controllerId: p.decisionOwnerControllerId, battleId: p.battleId };
    return {
...
export function advanceChoices(session) {
    const p = session.state.pendingDecision;
...
export function schwerpunktChoices(session) {
    const p = session.state.pendingDecision;
...
export function retreatPlan(session, presentation) {
    const p = session.state.pendingDecision;
    ...
    const sim = structuredClone(session.state), completeUnitIds = [], completeRoutes = [];
    let activeUnitId = null, options = [];
    for (const id of order) {
        ...
        for (const hex of path) {
            if (!getLegalRetreatStepOptions(sim, session.rules, unit).some(h => coreHexKey(h) === coreHexKey(hex))) {
                valid = false;
                break;
            }
            unit.hex = { ...hex };
        ...
        if (valid && path.length === maximum) {
            ...
            if (path.length > 0 || presentation.retreatOrder.includes(id))
                completeUnitIds.push(id);
            continue;
        }
        activeUnitId = id;
        if (valid && path.length < maximum) {
            ...
            options = getLegalRetreatStepOptions(sim, session.rules, unit).filter(hex => {
                const candidate = structuredClone(sim), mover = candidate.units[id];
                mover.hex = { ...hex };
    ...
    const action = { type: 'RETREAT', controllerId: p.decisionOwnerControllerId, battleId: p.battleId, retreats: order.map(unitId => ({ unitId, path: (presentation.retreatDrafts[unitId] ?? []).map(h => ({ ...h })) })) };
...
export function chooseRetreater(session, presentation, id) {
    const p = session.state.pendingDecision;
    if (p?.kind !== 'RETREAT' || presentation.privacyGate || p.decisionOwnerControllerId !== session.activeViewerControllerId || !p.unitIds.includes(id))
        return;
    const plan = retreatPlan(session, presentation);
    if (plan.completeUnitIds.includes(id))
        return;
    ...
    if (plan.activeUnitId && (presentation.retreatDrafts[plan.activeUnitId]?.length ?? 0) > 0)
        return;
    presentation.retreatOrder = [...planned.filter(candidate => candidate !== id), id];
    presentation.activeRetreaterId = id;
}
export function chooseRetreatDestination(session, presentation, destination) {
    const p = session.state.pendingDecision;
    if (p?.kind !== 'RETREAT' || presentation.privacyGate || p.decisionOwnerControllerId !== session.activeViewerControllerId)
        return;
    const plan = retreatPlan(session, presentation), id = plan.activeUnitId;
    if (!id || !plan.options.some(h => coreHexKey(h) === coreHexKey(destination))) {
        presentation.message = msg('feedback.illegalRetreat');
        return;
    }
    if (!presentation.retreatOrder.includes(id))
        presentation.retreatOrder.push(id);
    presentation.retreatDrafts[id] = [...(presentation.retreatDrafts[id] ?? []), { ...destination }];
    continueCombatFlow(session, presentation);
}
export function chooseAdvancer(session, presentation, unitId) {
    if (!presentation.privacyGate && session.state.pendingDecision?.decisionOwnerControllerId === session.activeViewerControllerId && advanceChoices(session).includes(unitId))
        presentation.advanceUnitId = unitId;
}
/** Counters can sit above decision polygons; route their taps to the same map action. */
export function routeCombatDecisionCounter(session, presentation, unitId) {
    const p = session.state.pendingDecision, unit = session.state.units[unitId];
    if (!p || !unit || presentation.privacyGate)
        return false;
    if (p.kind === 'RETREAT') {
        const plan = retreatPlan(session, presentation);
        if (plan.options.some(h => coreHexKey(h) === coreHexKey(unit.hex)))
            chooseRetreatDestination(session, presentation, unit.hex);
        else
            chooseRetreater(session, presentation, unitId);
        return true;
    }
    if (p.kind === 'ADVANCE_AFTER_COMBAT') {
        const target = session.state.combatTransactions[p.battleId]?.targetHex;
        if (target && coreHexKey(target) === coreHexKey(unit.hex))
            chooseAdvanceDestination(session, presentation, unit.hex);
        else
            chooseAdvancer(session, presentation, unitId);
        return true;
    }
    if (p.kind === 'SCHWERPUNKT_OPTION') {
        if (p.decisionOwnerControllerId === session.activeViewerControllerId && schwerpunktChoices(session).some(choice => coreHexKey(choice.target) === coreHexKey(unit.hex))) {
            presentation.schwerpunktTarget = { ...unit.hex };
            presentation.interactionMode = 'SCHWERPUNKT';
        }
        return true;
    }
    return false;
}
export function undoRetreatDestination(presentation) {
...
export function chooseAdvanceDestination(session, presentation, destination) {
    const p = session.state.pendingDecision;
    if (p?.kind !== 'ADVANCE_AFTER_COMBAT' || presentation.privacyGate || p.decisionOwnerControllerId !== session.activeViewerControllerId)
        return;
    ...
    if (!tx || !unitId || coreHexKey(destination) !== coreHexKey(tx.targetHex))
        return;
    ...
    if (!out.result.accepted) {
        presentation.message = joinIssues(out.result.issues);
        return;
    }
    continueCombatFlow(session, presentation);
}
...
export function continueCombatFlow(session, presentation) {
    for (let transitions = 0; transitions < 16; transitions++) {
        ...
        if (p.kind === 'DEFENDER_REACTION') {
            ...
            if (!choices.artillery.length && !choices.hq.length)
                forced = { ...base, type: 'PASS_REACTION' };
        }
        else if (p.kind === 'ADVANCE_AFTER_COMBAT') {
            const ids = advanceChoices(session);
            if (!ids.length)
                forced = { ...base, type: 'PASS_ADVANCE' };
            else if (ids.length === 1)
                presentation.advanceUnitId = ids[0];
        }
        else if (p.kind === 'RETREAT') {
            const plan = retreatPlan(session, presentation);
            forced = plan.action;
            presentation.activeRetreaterId = plan.activeUnitId;
            presentation.interactionMode = 'RETREAT';
        }
            ...
            if (!any)
                forced = { ...base, type: 'PASS_BREAKTHROUGH' };
            else if (!presentation.breakthroughUnitId)
                presentation.breakthroughUnitId = p.eligibleUnitIds[0] ?? null;
        }
        else if (p.kind === 'SCHWERPUNKT_OPTION' && !schwerpunktChoices(session).length)
            forced = { ...base, type: 'PASS_SCHWERPUNKT' };
        if (!forced) {
            ...
            setActiveViewer(session, p.decisionOwnerControllerId);
            presentation.privacyGate = null;
            return;
        }
