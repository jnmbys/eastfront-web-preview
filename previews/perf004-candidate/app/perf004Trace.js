...
async function run(limit){const status=bar.querySelector('#perf004-run-status'),record={start:now(),limit,lods:detail(),completed:0};batches.push(record);for(const b of bar.querySelectorAll('button'))b.disabled=true;try{
 for(let i=0;i<limit;i++){
  ...
  const before=count(),card=options[before];if(!card)throw Error('No observed plain destination');card.click();await pause(30);const confirm=document.querySelector('#confirm-deployment');if(!confirm||confirm.disabled)throw Error('Confirmation unavailable');confirm.click();
