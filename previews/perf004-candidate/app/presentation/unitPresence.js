...
import { PRESENCE_FAMILY, PRESENCE_LOD, PRESENCE_PALETTE, PRESENCE_PROFILE, presenceComposition } from './unitPresenceTypes.js';
...
export class UnitPresenceLayer {
    ...
    ghosts = new Map();
    lod = 'far';
    bind(counterLayer, identities, counters) {const __p4end=perf4.begin("presentation/unitPresence.js:UnitPresenceLayer.bind");try{
        const previous = new Map(this.units);
        this.unmount();
        if (!counterLayer?.parentNode)
            return;
        ...
        for (const identity of identities) {
            const counter = counters.get(identity.id);
            if (!counter || this.ghosts.has(identity.id))
                continue;
            const binding = this.create(identity, counter), old = previous.get(identity.id);
            if (old)
                binding.from = old.transform === binding.transform ? old.from : this.composition(old);
    ...
    reconcile(counterLayer, identities, counters, changed) {
        if (!this.layer || this.layer.parentNode !== counterLayer.parentNode) {
            this.bind(counterLayer, identities, counters);
            return;
        }
        ...
        for (const identity of identities) {
            ...
            if (!binding || changed.has(identity.id) || binding.identityKey !== `${identity.side}:${identity.type}`) {
                const old = binding;
                binding = this.create(identity, counter);
                if (old) {
                    binding.from = old.transform === binding.transform ? old.from : this.composition(old);
                    old.root.remove();
                }
                this.units.set(identity.id, binding);
                this.applyLod(binding);
            }
        ...
        for (const node of order) {
            if (node !== cursor)
                this.layer.insertBefore(node, cursor);
    ...
    capture(id) {
        ...
        if (!entry)
            return;
        entry.root.removeAttribute('data-presence-id');
        entry.root.setAttribute('data-presence-ghost', id);
        this.units.delete(id);
        this.ghosts.set(id, entry);
    }
    removeGhost(id) { this.ghosts.get(id)?.root.remove(); this.ghosts.delete(id); }
    setLod(lod, force = false) {
        if (lod === this.lod && !force)
            return;
        this.lod = lod;
    ...
    paint(id, state, transform, opacity) {
    ...
    restore(id) {
        ...
        if (!binding)
            return;
        binding.root.setAttribute('transform', binding.transform);
        binding.root.removeAttribute('data-presence-phase');
        binding.root.setAttribute('opacity', String(PRESENCE_LOD[this.lod].opacity));
        binding.body.setAttribute('transform', `scale(${binding.facing} 1)`);
        const { x, y, scale } = this.composition(binding);
        binding.pedestal.setAttribute('transform', `translate(${x} ${y}) scale(${scale})`);
        binding.flash.setAttribute('opacity', '0');
        binding.flash.setAttribute('transform', 'rotate(0)');
        binding.emphasis.setAttribute('opacity', '0');
        binding.emphasis.setAttribute('stroke', '#e0c98d');
    }
    dispose() { this.unmount(); for (const id of this.ghosts.keys())
        this.removeGhost(id); }
    unmount() {const __p4end=perf4.begin("presentation/unitPresence.js:UnitPresenceLayer.unmount");try{
        for (const ghost of this.ghosts.values())
            ghost.root.remove();
        this.layer?.remove();
    ...
    composition(binding) { return presenceComposition(binding.family, binding.half, binding.stackSign, binding.heavy, this.lod); }
    create(identity, counter) {const __p4end=perf4.begin("presentation/unitPresence.js:UnitPresenceLayer.create");try{
        ...
        if (identity.type === 'JAGER' || identity.type === 'ELITE_INFANTRY')
            node(doc, 'path', { d: 'M-8 8l8 2 8-2', fill: 'none', stroke: palette.accent, 'stroke-width': 1.3 }, detail);
