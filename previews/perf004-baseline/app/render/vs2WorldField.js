import { HEX_SIZE, SQRT3, hexPolygon, hexToPixel, hexNeighbors, sharedHexEdge } from '../geometry/hex.js';
export const VS2_WORLD_H = HEX_SIZE * SQRT3;
export const VS2_REGIONS = ['plain', 'forest', 'hill', 'rough', 'marsh', 'city', 'lake'];
const key = (x, y) => `${x},${y}`;
const smooth = (t) => { const c = Math.max(0, Math.min(1, t)); return c * c * (3 - 2 * c); };
export function vs2Region(terrain) {
    switch (terrain) {
...
export function createVS2RegionField(hexes) {
    const feather = VS2_WORLD_H * 0.22, buckets = new Map(), edges = new Map();
    ...
    for (const hex of sorted) {
        ...
        for (let by = Math.floor((center.y - HEX_SIZE) / VS2_WORLD_H); by <= Math.floor((center.y + HEX_SIZE) / VS2_WORLD_H); by++) {
            for (let bx = Math.floor((center.x - HEX_SIZE) / VS2_WORLD_H); bx <= Math.floor((center.x + HEX_SIZE) / VS2_WORLD_H); bx++) {
                const k = key(bx, by), list = buckets.get(k) ?? [];
                list.push(cell);
                buckets.set(k, list);
            }
        }
        for (const neighbor of hexNeighbors(hex.coord)) {
            ...
            if (right === region || (right !== -1 && (hex.coord.q > neighbor.q || (hex.coord.q === neighbor.q && hex.coord.r > neighbor.r))))
                continue;
            const segment = sharedHexEdge(hex.coord, neighbor);
            const edge = { a: segment[0], b: segment[1], left: region, right };
            boundaryCount++;
            for (let by = Math.floor((Math.min(edge.a.y, edge.b.y) - feather) / VS2_WORLD_H); by <= Math.floor((Math.max(edge.a.y, edge.b.y) + feather) / VS2_WORLD_H); by++) {
                for (let bx = Math.floor((Math.min(edge.a.x, edge.b.x) - feather) / VS2_WORLD_H); bx <= Math.floor((Math.max(edge.a.x, edge.b.x) + feather) / VS2_WORLD_H); bx++) {
                    const k = key(bx, by), list = edges.get(k) ?? [];
                    list.push(edge);
                    edges.set(k, list);
                }
            }
    ...
    return {
        ...
        sample(x, y) {
            ...
            const weights = VS2_REGIONS.map((_, i) => i === owner ? 1 : 0);
            let coverage = owner < 0 ? 0 : 1;
            for (const edge of edges.get(k) ?? []) {
                const d = distance(edge, x, y);
                if (d >= feather)
                    continue;
                ...
                if (edge.right < 0)
                    coverage = owner < 0 ? Math.max(coverage, outside) : Math.min(coverage, 1 - outside);
            }
...
export function vs2VisualValue(seed, layer, x, y) {
...
export function vs2WorldNoise(seed, layer, x, y, period) {
