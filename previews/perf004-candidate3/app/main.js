...
function refreshDynamicView() {const __p4end=perf4.begin("main.js:refreshDynamicView");try{
    ...
    const retainedPanel = deploymentPanelRenderer.update(panel, session, model, presentation.selectedDeploymentUnitId, deploymentTouch, locations => sidePanelMarkup(model, locations), () => {
        ...
        panelScroll = perf4.sync('layout:read-panelScroll',()=>panel.querySelector('.command-panel-scroll')?.scrollTop ?? 0);
        rosterScroll = perf4.sync('layout:read-rosterScroll',()=>panel.querySelector('.roster-list')?.scrollTop ?? 0);
        locationScroll = perf4.sync('layout:read-locationScroll',()=>panel.querySelector('.location-grid')?.scrollTop ?? 0);
    });
