...
function startNewGame() {const __end=__trace.begin("startNewGame");try{ deploymentTouch = createDeploymentTouch(); if (!productionMap || !cachedTerrainSurface) {
    appStatus = 'FATAL';
...
async function boot(networkModel) {const __end=__trace.begin("boot");try{
    ...
    try {
        ...
        for (const lod of ['far', 'medium', 'close']) {
            cachedTerrainSurface = await __trace.async(`surface:${lod}`,()=>buildCachedTerrainSurface(terrainModel, TERRAIN_VISUAL_SEED, 'p5', lod, vs2.worldBase));
