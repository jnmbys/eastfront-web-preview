...
export function planVS2Forest(projection, seed, assets = vs2AssetCatalog) {
    ...
    const step = VS2_WORLD_H * 0.8, bounds = projection.viewBox, result = [];
    if (!entries.length)
        return result;
    for (let iy = Math.floor(bounds.minY / step); iy <= Math.ceil((bounds.minY + bounds.height) / step); iy++) {
        for (let ix = Math.floor(bounds.minX / step); ix <= Math.ceil((bounds.minX + bounds.width) / step); ix++) {
            const random = (purpose) => vs2VisualValue(seed, `forest:${purpose}`, ix, iy);
            const x = (ix + 0.5 + (random('x') - 0.5) * 0.45) * step, y = (iy + 0.5 + (random('y') - 0.5) * 0.45) * step;
            if (projection.field.sample(x, y).weights[1] < 0.15)
                continue;
...
export function createVS2ForestCoverage(projection) {
...
export async function paintVS2Forest(ctx, projection, seed, lod, assets = vs2AssetCatalog) {
    ...
    layer.width = bounds.width / pixel;
    layer.height = bounds.height / pixel;
    const target = layer.getContext('2d', { willReadFrequently: true });
    if (!target)
        throw new Error('VS2 canopy Canvas 2D unavailable');
    ...
    try {
        for (const id of ids) {
            ...
            if (!entry.LOD.includes(lod) || !entry.rotationAllowed.degrees?.includes(0))
                throw new Error(`VS2 canopy transform/LOD mismatch: ${id}`);
            ...
            try {
                for (const p of plan.filter(p => p.assetId === id))
                    target.drawImage(image.source, (p.x - p.width * entry.anchor[0] - bounds.minX) / pixel, (p.y - p.height * entry.anchor[1] - bounds.minY) / pixel, p.width / pixel, p.height / pixel);
            }
            finally {
                image.release?.();
            }
