...
/** Keep each existing vector paint pass in one Canvas transaction. Yielding while
 * a transformed path is open changes browser antialias/compositing roundoff. */
export function measureTerrainWork(stage, work) {
    const start = performance.now();
    try {
        return work();
    }
        ...
        for (const observer of observers) {
            try {
                observer({ stage, elapsedMs, workMs: elapsedMs, maxSliceMs: elapsedMs, yields: 0 });
            }
            catch { }
        }
    }
