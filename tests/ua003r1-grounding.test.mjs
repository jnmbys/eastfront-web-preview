...
test('UA003R1 stack retains source anchors, separates the models and leaves a visible ground gap',()=>{
 ...
 for(const lod of ['medium','close']){level(h,lod);const comps=['g','p'].map((id,i)=>{const p=presence(h,id),counter=h.dom().counter(id),c=composition(p);assert.equal(counter.getAttribute('transform'),anchors[i]);assert.equal(p.getAttribute('transform'),anchors[i]);const contact=p.querySelector('[data-presence-contact]').querySelectorAll('ellipse')[0],shadowBottom=c.y+(Number(contact.getAttribute('cy'))+Number(contact.getAttribute('ry')))*c.scale,half=Number(counter.querySelector('.counter-body').getAttribute('width'))/2;if(c.y<0)assert(shadowBottom<-half-1,'Rear shadow must end on terrain before the Counter border');else assert(c.y-(id==='g'?21:18)*c.scale>half+5,'Front hull must clear the Counter lower border');return c;});assert(comps[0].x*comps[1].x<0);assert(Math.abs(comps[0].x-comps[1].x)>=40);assert(comps[0].y>0&&comps[1].y>0,'Stack models occupy the near ground strip without crossing their Counter faces');}h.runtime.dispose();
});

test('UA003R1 major family profiles occupy readable Medium/Close widths without spanning a full adjacent hex',()=>{
 for(const [id,t]of Object.entries(defaultRules.unitTemplates)){const h=harness(movementFixture([unit('g',id,t.side,t.type,{q:0,r:0})]));let previous=0;for(const lod of ['medium','close']){level(h,lod);const p=presence(h,'g'),profile=PRESENCE_PROFILE[p.getAttribute('data-presence-family')],width=profile.width*composition(p).scale,counterWidth=Number(h.dom().counter('g').querySelector('.counter-body').getAttribute('width'));assert(width>counterWidth*(lod==='medium'?1:1.3));assert(width<counterWidth*(lod==='medium'?1.4:1.8));assert(width<90);assert(width>previous);previous=width;}h.runtime.dispose();}
});
