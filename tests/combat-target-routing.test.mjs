...
test('enemy hex follows same route; illegal enemy and empty target rejected',()=>{const {s,p}=setup();assert.equal(routeCombatTarget(s,p,{q:0,r:0}),true);const before={...p.attackTarget};selectCounter(s,p,'far');assert.deepEqual(p.attackTarget,before);assert.equal(p.selectedUnitId,'g');assert.match(formatMessage(p.message),/Cannot select attack target/);routeCombatTarget(s,p,{q:2,r:2});assert.deepEqual(p.attackTarget,before);assert(combatTargetIssues(s,p,{q:4,r:0}).length>0);});
