/** Render the saved audit, without collecting further samples or timings. */
import {readFileSync,writeFileSync} from 'node:fs';
import {gunzipSync} from 'node:zlib';
const a=JSON.parse(gunzipSync(readFileSync('evidence/mp-005a/audit.json.gz')));
const table=(head,rows)=>`| ${head.join(' | ')} |\n| ${head.map(()=>'---').join(' | ')} |\n${rows.map(r=>`| ${r.join(' | ')} |`).join('\n')}`;
const n=x=>Number(x).toLocaleString('en-US',{maximumFractionDigits:2});
const metric=x=>`${n(x.median)} / ${n(x.max)}`;
const label={'SOVIET-actor':'苏军部署：操作方（苏）','SOVIET-waiting':'苏军部署：等待方（德）','GERMAN-actor':'德军部署：操作方（德）','GERMAN-waiting':'德军部署：等待方（苏）'};
const first=a.rows.find(r=>r.label==='SOVIET-deployment'&&r.role==='actor'&&r.matchRevision===10);
const waiting=a.rows.find(r=>r.label==='SOVIET-deployment'&&r.role==='waiting'&&r.matchRevision===10);
const cats={map:'地图 hexes + edges（三份）',unitsAndCounters:'units（两份）+ counters',visibility:'identified/contactHexKeys（各两份）',contactsAndLastKnown:'CONTACT / Last Known（各两份）',deployment:'部署 roster / zone / 状态',combatIncludingHistory:'combat（本例 null；含 history）',events:'授权表现事件（本例 []）',otherAndJsonSyntax:'其余字段、信封及 JSON 语法'};
const path=m=>Object.fromEntries(m.fields.map(f=>[f.path,f.bytes]));
const f=path(first),w=path(waiting);
const output=`# MP-005A — Snapshot Payload Audit checkpoint
...
- ${a.environment.node}, ${a.environment.platform}/${a.environment.arch}, ${a.environment.cpu}。本地 Node 内存 authority；正式 Scenario 固定测试 seed 17；不连接公网、不创建线上房间。
- 一组完整部署序列：苏军 32、德军 26 次，操作方/等待方各一份，共 116 部署快照。另覆盖初始、阶段交接、移动、多攻击者战斗、防守反应、resync，以及受控 CONTACT→Last Known / 隐藏炮兵夹具；合计 ${a.counts.snapshotSamples} 授权快照、${a.counts.acceptedActions} accepted actions。
...
代表样本：苏军第 10 次部署、revision 10；操作方 ${n(first.bytes)} B，等待方 ${n(waiting.bytes)} B。双方各用自己的完整授权 DTO。

${table(['不重叠分类','操作方 B','占比','等待方 B','占比'],Object.entries(cats).map(([k,title])=>[title,n(first.categories[k]),n(first.categories[k]/first.bytes*100)+'%',n(waiting.categories[k]),n(waiting.categories[k]/waiting.bytes*100)+'%']))}

关键独立字段（下面是嵌套明细，不再与上表重复相加）：

${table(['路径','操作方 B','等待方 B'],['payload.view.hexes','payload.view.edges','payload.view.units','payload.model.playerView.hexes','payload.model.playerView.edges','payload.model.playerView.units','payload.model.hexes','payload.model.edges','payload.model.counters','payload.model.deployment.roster','payload.model.deployment.zoneKeys'].map(p=>[p,n(f[p]),n(w[p])]))}

一份 hexes=${n(f['payload.view.hexes'])} B，一份 edges=${n(f['payload.view.edges'])} B；每条消息各带三份，地图合计 ${n(first.categories.map)} B。**测量后确认重复地图最大**，不是先验假设。
...
${table(['部署组','样本','不变 value 字节占比：中位 / 最大','消息 B：首条 → 末条'],Object.entries(a.summary).map(([k,s])=>{const [side,role]=k.split('-'),rs=a.rows.filter(r=>r.label===side+'-deployment'&&r.role===role);return [label[k],s.bytes.n,metric(s.unchangedValuePercent)+'%',n(rs[0].bytes)+' → '+n(rs.at(-1).bytes)];}))}
...
${table(['部署组 / 每接收方','N','原 B 中位 / 最大','提案 B 中位 / 最大','逐样本减量% 中位 / 最大'],Object.entries(a.summary).map(([k,s])=>[label[k],s.bytes.n,metric(s.bytes),metric(s.proposedBytes),metric(s.savedPercent)]))}

这是假设未来新增格式后，实际离线编码得到的 JSON 大小；当前客户端不能直接消费，不接入生产。消息数量仍为每 accepted action 各接收方 1 snapshot；本组 ${a.counts.acceptedActions} 次 action 均如此，查询消息 0。没有省略必要快照或改变 ACK。

## CPU 对照（本地 Node，ms 中位 / 最大）

${table(['部署组','现行 stringify','提案 pack + stringify','现行 JSON.parse','提案 parse + 深拷贝重建'],Object.entries(a.summary).map(([k,s])=>[label[k],metric(s.cpu.originalStringifyMs),metric(s.cpu.proposalPackStringifyMs),metric(s.cpu.originalParseMs),metric(s.cpu.proposalParseCloneDecodeMs)]))}
...
`;
writeFileSync('evidence/mp-005a/README.md',output);
