...
import {decodeSnapshot,MAP_SNAPSHOT as mapFormat,COMPACT_SNAPSHOT as compact,FULL_SNAPSHOT as full} from '../../.server-dist/src/multiplayer/snapshotCodec.js';
...
try{
 ...
 async function peer(token=null,negotiate=false,preferred=compact){
  ...
  if(negotiate){const selected=await send('SET_SNAPSHOT_FORMAT',{format:preferred});if(selected.messageType!=='SNAPSHOT_FORMAT_SELECTED'||selected.payload.format!==preferred)throw Error('Compact negotiation failed');}
 ...
 async function deploy(p,index,expected=compact){const unit=current.model.deployment.roster.find(u=>!u.placed),[q,r]=current.model.deployment.zoneKeys[index].split(',').map(Number);
  ...
  current=decodeSnapshot(actor.m.payload,true,true);const other=decodeSnapshot(waiter.m.payload,false);
  if(actor.m.payload.format!==expected||waiter.m.payload.format!==full||current.serverSequence!==ack.payload.serverSequence+1||other.view.units.length!==0||current.view.units.length!==index+1)throw Error('Format/order/privacy/count mismatch');
 ...
 const selected=await restored.send('SET_SNAPSHOT_FORMAT',{format:mapFormat});if(selected.messageType!=='SNAPSHOT_FORMAT_SELECTED'||selected.payload.format!==mapFormat)throw Error('Map negotiation failed');
 await deploy(restored,2,mapFormat);
 await restored.send('LEAVE_ROOM');report.status='PASS';report.reconnect={fullRecovery:true,revision:1,continuedRevision:3};
