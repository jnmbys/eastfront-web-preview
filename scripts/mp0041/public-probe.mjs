...
try{
 ...
 async function peer(compression){
  const ws=new WebSocket(url,{origin,perMessageDeflate:compression}),messages=[],frames=[];sockets.push(ws);let pending=Buffer.alloc(0);
  ...
  async function request(type,payload={}){const id=randomUUID(),sent=performance.now();ws.send(JSON.stringify({protocolVersion:2,messageType:type,requestId:id,payload}));return {...await wait(m=>m.requestId===id),id,sent};}
  const hello=await request('HELLO',{displayName:'MP0041 public probe'});connection.connectionId=hello.m.payload.connectionId;
  ...
  connection.server=await diagnostic();return{ws,messages,frames,request,wait,diagnostic,connection};
 }
