  ...
  bind(root:ParentNode|null,identities:readonly UnitPresenceIdentity[]=[]):void {
    ...
    if(sameRoot){
      ...
      if(layer&&layer===this.layer){
        ...
        for(const [id,binding]of this.bindings)if(current.get(id)!==binding.counter){this.bindings.delete(id);changed.add(id);}
        for(const [id,counter]of current)if(!this.ghosts.has(id)&&!this.bindings.has(id)){this.bindings.set(id,this.binding(counter,hits.get(id)));changed.add(id);}
        this.presence.reconcile(layer,identities,new Map([...this.bindings].map(([id,b])=>[id,b.counter])),changed);
        this.displayedMode='';return;
      }
    }
