import type { Point } from '../geometry/hex.js';
import type { CueCharacter, TravelKind } from './events.js';
...
} satisfies Record<CueCharacter,{recoil:number;reach:number}>);
...
export function travelEase(kind:TravelKind,t:number):number {
...
export function travelAccent(kind:TravelKind,progress:number,direction:Point){
...
export function fireStagger(count:number):number {return Math.min(T.FIRE_STAGGER_CAP,Math.max(0,count-1)*T.FIRE_STAGGER);}
export function fireDelay(index:number,count:number):number {return count>1?index*fireStagger(count)/(count-1):0;}
export function cueAccent(phase:'windup'|'firing'|'hit'|'destroyed',progress:number,direction:Point,character:CueCharacter){
