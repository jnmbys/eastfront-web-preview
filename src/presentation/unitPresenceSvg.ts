import type { PresenceFamily } from './unitPresenceTypes.js';

const NS='http://www.w3.org/2000/svg';
export function presenceNode(doc:Document,tag:string,attrs:Record<string,string|number>,parent?:Element):Element {
  const node=doc.createElementNS(NS,tag);
  for(const [key,value] of Object.entries(attrs))node.setAttribute(key,String(value));
  node.setAttribute('pointer-events','none');parent?.appendChild(node);return node;
}
...
const formation=[[-18,0],[0,-5],[19,2]] as const;
const artwork:Record<PresenceFamily,{side:string;top:string;detail:string;facet?:string}>={
  infantry:{
...
export function createPresenceDefinitions(doc:Document):Element {
  ...
  for(const [family,art]of Object.entries(artwork)){
    ...
    if(art.facet)presenceNode(doc,'path',{d:art.facet,fill:'#1a2426','fill-opacity':.43},body);
