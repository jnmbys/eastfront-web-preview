...
function bindDynamic(model?:BrowserRenderModel):void{
  ...
  document.querySelectorAll<SVGPolygonElement>('[data-role="deployment-hex"]').forEach((element)=>{const key=element.dataset.hex;if(!key)return;const action=()=>{chooseTouchTarget(key);};element.addEventListener('click',action);bindKeyboardActivation(element,action);});document.querySelectorAll<SVGPolygonElement>('[data-role="move-option"]').forEach((element)=>{const key=element.dataset.hex;if(!key)return;const action=()=>{extendMoveDraft(session!,presentation,parseHex(key));refreshDynamicView();};element.addEventListener('click',action);bindKeyboardActivation(element,action);});
