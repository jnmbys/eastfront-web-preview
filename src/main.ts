...
import {
  advanceAfterCombat,appendLossDraft,cancelMoveDraft,cancelRailRepair,clearAttackDraft,clearLossDraft,commitBreakthrough,commitLosses,commitMoveDraft,commitRailRepair,commitRetreat,commitSchwerpunkt,confirmPrivacyGate,declareAttack,deploySelectedReinforcement,deploySelectedUnit,
  enterRailRepairMode,entrenchSelectedUnit,extendBreakthroughDraft,extendMoveDraft,extendRetreatDraft,passAdvance,passBreakthrough,passCombatReaction,passSchwerpunkt,readyForPhase,recoverSelectedUnit,routeCombatTarget,isCombatTargetSelection,combatTargetIssues,selectAttackerArtillery,selectBreakthroughUnit,selectCounter,selectDeploymentRosterUnit,
  selectRailEngineer,selectReinforcement,selectRetreater,selectSchwerpunktTarget,switchViewerForDevelopment,toggleAttackUnit,toggleRailRepairEdge,undoBreakthroughDraft,undoLossDraft,undoMoveDraft,undoRetreatStep,useDefenderArtillery,
...
function issueHtml(issues:readonly {code:string;message:string}[]):string{return issues.length?`<div class="issue-list">${issues.map((issue)=>`<span>${esc(issue.code)} · ${esc(issue.message)}</span>`).join('')}</div>`:'<div class="issue-list"><span>Core preview: legal</span></div>';}
...
function selectedSummary(model:BrowserRenderModel):string{
 ...
 if(!unit)return '<div class="command-empty"><span class="command-reticle" aria-hidden="true">◇</span><p>Select a unit to inspect strength, supply and movement.</p></div>';
...
function mapRenderOptions(model:BrowserRenderModel,lodOverride?:TerrainLod):CoreSvgOptions{
  ...
  const rendererMode=developerUi?presentation.rendererMode:'production';return {debug:developerUi&&presentation.debug,rendererMode,assetSet:'p5',lod,scenarioSeed:session!.state.random.seed,staticTerrainSurface:rendererMode==='production'};
}
...
function render():void{
  ...
  if(appStatus==='LOADING'){root.innerHTML=loadingMarkup();return;}
  if(appStatus==='FATAL'){root.innerHTML=fatalMarkup(fatalMessage||'Required production resources could not be initialized.');bind();return;}
  if(appStatus==='HOME'){root.innerHTML=homeMarkup(profile);bind();return;}
  if(!session){appStatus='FATAL';fatalMessage='No active production session.';root.innerHTML=fatalMarkup(fatalMessage);bind();return;}
...
function paintCombatTargets():void{
  ...
  if(enabled)for(const unit of Object.values(session.state.units)){
    if(unit.alive&&unit.side!==session.state.activeSide){const key=coreHexKey(unit.hex);if(!legal.has(key)&&combatTargetIssues(session,presentation,unit.hex).length===0)legal.add(key);}
  }
...
function bindDynamic():void{
  ...
  document.querySelector<HTMLButtonElement>('#confirm-deployment')?.addEventListener('click',event=>{
   const button=event.currentTarget as HTMLButtonElement;button.disabled=true;button.textContent='DEPLOYING…';
  ...
  document.querySelector('#move-undo')?.addEventListener('click',()=>{undoMoveDraft(presentation);render();});document.querySelector('#move-cancel')?.addEventListener('click',()=>{cancelMoveDraft(presentation);render();});document.querySelector('#move-commit')?.addEventListener('click',()=>{commitMoveDraft(session!,presentation);render();});document.querySelector('#recover-unit')?.addEventListener('click',()=>{recoverSelectedUnit(session!,presentation);render();});document.querySelector('#entrench-unit')?.addEventListener('click',()=>{entrenchSelectedUnit(session!,presentation);render();});
  document.querySelectorAll<HTMLButtonElement>('[data-deploy-destination]').forEach(element=>element.addEventListener('click',()=>{const key=element.dataset.deployDestination;if(!key)return;chooseTouchTarget(key);}));
  document.querySelectorAll<HTMLElement>('[data-deploy-unit-id]').forEach((element)=>{const id=element.dataset.deployUnitId;if(!id)return;const action=()=>{deploymentTouch=createDeploymentTouch();selectDeploymentRosterUnit(presentation,id);refreshDynamicView();};element.addEventListener('click',action);bindKeyboardActivation(element,action);});
  document.querySelectorAll<SVGGElement>('[data-unit-id]').forEach((element)=>{const id=element.dataset.unitId;if(!id)return;const action=()=>{if(chooseCounterTarget(id))return;selectCounter(session!,presentation,id);refreshDynamicView();};element.addEventListener('click',(event)=>{event.stopPropagation();action();});bindKeyboardActivation(element,action);});document.querySelectorAll<SVGRectElement>('[data-hit-unit-id]').forEach((element)=>{const id=element.dataset.hitUnitId;if(!id)return;element.addEventListener('click',(event)=>{event.stopPropagation();if(chooseCounterTarget(id))return;selectCounter(session!,presentation,id);refreshDynamicView();});});
  document.querySelectorAll<SVGPolygonElement>('[data-role="deployment-hex"]').forEach((element)=>{const key=element.dataset.hex;if(!key)return;const action=()=>{chooseTouchTarget(key);};element.addEventListener('click',action);bindKeyboardActivation(element,action);});document.querySelectorAll<SVGPolygonElement>('[data-role="move-option"]').forEach((element)=>{const key=element.dataset.hex;if(!key)return;const action=()=>{extendMoveDraft(session!,presentation,parseHex(key));render();};element.addEventListener('click',action);bindKeyboardActivation(element,action);});
  ...
  document.querySelector('#attack-toggle-selected')?.addEventListener('click',()=>{if(presentation.selectedUnitId)toggleAttackUnit(session!,presentation,presentation.selectedUnitId);render();});document.querySelector('#attack-clear')?.addEventListener('click',()=>{clearAttackDraft(presentation);render();});document.querySelector('#attack-declare')?.addEventListener('click',()=>{declareAttack(session!,presentation);render();});document.querySelector('#attack-art-none')?.addEventListener('click',()=>{selectAttackerArtillery(presentation,null);render();});document.querySelectorAll<HTMLElement>('[data-attack-artillery]').forEach((el)=>el.addEventListener('click',()=>{selectAttackerArtillery(presentation,el.dataset.attackArtillery??null);render();}));
  document.querySelectorAll<SVGPolygonElement>('[data-role="attack-target"]').forEach((el)=>{const action=()=>{const key=el.dataset.hex;if(key){routeCombatTarget(session!,presentation,parseHex(key));refreshDynamicView();}};el.addEventListener('click',action);bindKeyboardActivation(el,action);});document.querySelector('#pass-reaction')?.addEventListener('click',()=>{passCombatReaction(session!,presentation);render();});document.querySelectorAll<HTMLElement>('[data-defender-artillery]').forEach((el)=>el.addEventListener('click',()=>{const id=el.dataset.defenderArtillery;if(id){useDefenderArtillery(session!,presentation,id);render();}}));
  document.querySelectorAll<HTMLElement>('[data-loss-unit]').forEach((el)=>el.addEventListener('click',()=>{const id=el.dataset.lossUnit;if(id){appendLossDraft(session!,presentation,id);render();}}));document.querySelector('#loss-undo')?.addEventListener('click',()=>{undoLossDraft(presentation);render();});document.querySelector('#loss-clear')?.addEventListener('click',()=>{clearLossDraft(presentation);render();});document.querySelector('#loss-commit')?.addEventListener('click',()=>{commitLosses(session!,presentation);render();});
  document.querySelectorAll<HTMLElement>('[data-retreater]').forEach((el)=>el.addEventListener('click',()=>{const id=el.dataset.retreater;if(id){selectRetreater(presentation,id);render();}}));document.querySelectorAll<HTMLElement|SVGPolygonElement>('[data-role="retreat-option"], [data-retreat-destination]').forEach((el)=>{const action=()=>{const key=el.dataset.hex??el.dataset.retreatDestination;if(key){extendRetreatDraft(session!,presentation,parseHex(key));refreshDynamicView();}};el.addEventListener('click',action);bindKeyboardActivation(el,action);});document.querySelector('#retreat-undo')?.addEventListener('click',()=>{undoRetreatStep(presentation);render();});document.querySelector('#retreat-commit')?.addEventListener('click',()=>{commitRetreat(session!,presentation);render();});
  document.querySelectorAll<HTMLElement>('[data-advance-unit]').forEach((el)=>el.addEventListener('click',()=>{const id=el.dataset.advanceUnit;if(id){advanceAfterCombat(session!,presentation,id);render();}}));document.querySelector('#pass-advance')?.addEventListener('click',()=>{passAdvance(session!,presentation);render();});document.querySelectorAll<HTMLElement>('[data-breakthrough-unit]').forEach((el)=>el.addEventListener('click',()=>{const id=el.dataset.breakthroughUnit;if(id){selectBreakthroughUnit(presentation,id);render();}}));document.querySelectorAll<SVGPolygonElement>('[data-role="breakthrough-option"]').forEach((el)=>el.addEventListener('click',()=>{const key=el.dataset.hex;if(key){extendBreakthroughDraft(session!,presentation,parseHex(key));render();}}));document.querySelector('#breakthrough-undo')?.addEventListener('click',()=>{undoBreakthroughDraft(presentation);render();});document.querySelector('#breakthrough-commit')?.addEventListener('click',()=>{commitBreakthrough(session!,presentation);render();});document.querySelector('#pass-breakthrough')?.addEventListener('click',()=>{passBreakthrough(session!,presentation);render();});document.querySelectorAll<SVGPolygonElement>('[data-role="schwerpunkt-target"]').forEach((el)=>el.addEventListener('click',()=>{const key=el.dataset.hex;if(key){selectSchwerpunktTarget(presentation,parseHex(key));render();}}));document.querySelectorAll<HTMLElement>('[data-schwerpunkt-unit]').forEach((el)=>el.addEventListener('click',()=>{const id=el.dataset.schwerpunktUnit;if(id){commitSchwerpunkt(session!,presentation,id);render();}}));document.querySelector('#pass-schwerpunkt')?.addEventListener('click',()=>{passSchwerpunkt(session!,presentation);render();});
