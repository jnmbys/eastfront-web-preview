...
function bindDynamic():void{
  ...
  if(session&&deriveBrowserRenderModel(session,presentation).readOnly)return;
  paintCombatTargets();
  ...
  document.querySelectorAll<SVGGElement>('[data-unit-id]').forEach((element)=>{const id=element.dataset.unitId;if(!id)return;const action=()=>{if(chooseCounterTarget(id))return;if(!routeCombatDecisionCounter(session!,presentation,id))selectCounter(session!,presentation,id);showCombatView();};element.addEventListener('click',(event)=>{event.stopPropagation();action();});bindKeyboardActivation(element,action);});document.querySelectorAll<SVGRectElement>('[data-hit-unit-id]').forEach((element)=>{const id=element.dataset.hitUnitId;if(!id)return;element.addEventListener('click',(event)=>{event.stopPropagation();if(chooseCounterTarget(id))return;if(!routeCombatDecisionCounter(session!,presentation,id))selectCounter(session!,presentation,id);showCombatView();});});
