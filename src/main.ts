...
function bindDynamic():void{
  ...
  document.querySelectorAll<HTMLElement>('[data-retreater]').forEach((el)=>el.addEventListener('click',()=>{const id=el.dataset.retreater;if(id){selectRetreater(presentation,id);render();}}));document.querySelectorAll<SVGPolygonElement>('[data-role="retreat-option"]').forEach((el)=>el.addEventListener('click',()=>{const key=el.dataset.hex;if(key){extendRetreatDraft(session!,presentation,parseHex(key));render();}}));document.querySelector('#retreat-undo')?.addEventListener('click',()=>{undoRetreatStep(presentation);render();});document.querySelector('#retreat-commit')?.addEventListener('click',()=>{commitRetreat(session!,presentation);render();});
