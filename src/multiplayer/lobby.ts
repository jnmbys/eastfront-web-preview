...
export function addMultiplayerHomeButton(root:HTMLElement,onOpen:()=>void):void {
  if(!document.querySelector('link[data-multiplayer-style]')){
    const link=document.createElement('link');link.rel='stylesheet';link.href='./multiplayer.css';link.dataset.multiplayerStyle='true';document.head.append(link);
  }
  const anchor=root.querySelector('#new-game-button');if(!anchor)return;
...
export function mountLobby(root:HTMLElement,onBack:()=>void,onMatch?:(client:LobbyClient)=>void):()=>void {
  ...
  const draw=()=>{
    ...
    const input=root.querySelector<HTMLInputElement>('#mp-name');input?.addEventListener('input',()=>{name=input.value;});
    const codeInput=root.querySelector<HTMLInputElement>('#mp-code');codeInput?.addEventListener('input',()=>{code=codeInput.value;});
    root.querySelector('#mp-connect')?.addEventListener('click',()=>{
      if(!name.trim()){input?.focus();return;}try{sessionStorage.setItem('eastfront.mp.name',name.trim());}catch{}client?.connect(name);
    });
    ...
    root.querySelectorAll<HTMLElement>('[data-mp-seat]').forEach(button=>button.addEventListener('click',()=>client?.send('SELECT_SEAT',{seat:button.dataset.mpSeat as 'GERMANY'|'SOVIET'})));
    ...
    root.querySelector('#mp-copy')?.addEventListener('click',()=>{
      const value=room?.roomCode;if(!value)return;
      void (async()=>{try{await navigator.clipboard.writeText(value);notice='copied';}catch{notice='copyHelp';}draw();})();
    });
  };
  const dispose=()=>{disposed=true;client?.dispose();window.removeEventListener('pagehide',pagehide);};
  const pagehide=()=>client?.dispose();window.addEventListener('pagehide',pagehide);
  draw();
  void loadMultiplayerUrl().then(result=>{if(disposed)return;url=result;loading=false;if(!url)notice='noService';else client=new LobbyClient(url,draw);draw();}).catch(()=>{if(!disposed){loading=false;notice='invalidUrl';draw();}});
  return dispose;
}
