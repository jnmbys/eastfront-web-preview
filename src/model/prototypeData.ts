import { hexKey, offsetToAxial, type HexCoord } from '../geometry/hex.js';

export type Faction = 'german' | 'soviet';
export type UnitKind = 'infantry' | 'armor' | 'artillery' | 'tank' | 'at';
export type Terrain = 'plain' | 'forest' | 'city' | 'marsh' | 'hill';
...
export const mapHexes: HexCoord[] = Array.from({ length: ROWS }, (_, row) =>
  Array.from({ length: COLS }, (_, col) => offsetToAxial(col, row)),
).flat();

export const mapHexKeys = new Set(mapHexes.map(hexKey));

function h(col: number, row: number): HexCoord {
  return offsetToAxial(col, row);
}

const terrainEntries: Array<[HexCoord, Terrain]> = [
...
export const terrainByHex = new Map<string, Terrain>(terrainEntries.map(([hex, terrain]) => [hexKey(hex), terrain]));
...
export const reachableHexes: HexCoord[] = [h(3,2),h(4,2),h(4,3),h(3,4),h(4,4),h(5,4),h(5,3)];
export const enemyZocHexes: HexCoord[] = [h(5,1),h(6,1),h(7,1),h(5,2),h(7,2),h(5,3),h(6,3)];
export const attackTargetHexes: HexCoord[] = [h(6,2), h(7,3)];

export const roadPath: HexCoord[] = [h(0,3),h(1,3),h(2,3),h(3,3),h(4,3),h(5,3),h(6,3),h(7,3),h(8,3),h(9,3)];
export const railPath: HexCoord[] = [h(4,0),h(4,1),h(5,2),h(5,3),h(6,4),h(6,5),h(6,6),h(7,6),h(7,7)];

// River segments live on shared hex edges, never through centers.
export const riverEdges: EdgeFeature[] = [
  {a:h(5,0), b:h(6,0)},
  {a:h(5,1), b:h(6,0)},
  {a:h(5,1), b:h(6,1)},
  {a:h(6,2), b:h(6,1)},
...
export const bridgeEdges: EdgeFeature[] = [
  {a:h(5,3), b:h(6,3)}, // road crossing
  {a:h(6,6), b:h(7,6)}, // railway crossing
];

export function terrainAt(hex: HexCoord): Terrain {
