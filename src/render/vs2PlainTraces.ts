  ...
  try {
    for (let iy = Math.floor(bounds.minY / step); iy < (bounds.minY + bounds.height) / step; iy++) {
      for (let ix = Math.floor(bounds.minX / step); ix < (bounds.minX + bounds.width) / step; ix++) {
        ...
        if ([-1, 0, 1].some(dx => [-1, 0, 1].some(dy => p.field.sample(x + dx * halfX, y + dy * halfY).weights[0]! < 0.96))) continue;
        if (p.corridors.some(e => vs2SegmentDistance({ x, y }, e.a, e.b) < Math.hypot(w, h) / 2 + e.radius)) continue;
        ...
        if (r('farm') > 0.955) {
          ...
          ctx.fillStyle = '#726b4e'; ctx.fillRect(-w / 2 + 3, -h / 2 + 3, 4.8, 3.4);
          ctx.fillStyle = '#a87c4f'; ctx.fillRect(-w / 2 + 2.5, -h / 2 + 2.5, 4.8, 1.7);
        }
        ctx.rotate(0.18); ctx.translate(-x, -y);
      }
    }
  } finally { ctx.restore(); }
}
  ...
  try {
    ...
    for (let iy = Math.floor(b.minY / step); iy < (b.minY + b.height) / step; iy++) {
      for (let ix = Math.floor(b.minX / step); ix < (b.minX + b.width) / step; ix++) {
        ...
        if (marshWeight < 0.55 || p.field.regionAt(x, y) !== 'marsh') continue;
        const environment = vs2MarshEnvironment(seed, x, y);
        if (r < 0.25 + (1 - environment.reeds) * 0.6) continue;
        if (p.corridors.some(e => vs2SegmentDistance({ x, y }, e.a, e.b) < e.radius + 4)) continue;
        ctx.globalAlpha = 0.22; ctx.fillStyle = environment.mud > 0.5 ? '#887550' : '#a29b60';
  ...
  try {
    for (const e of rivers) {
      ...
      for (let i = 0; i < 2; i++) for (const side of [-1, 1]) {
        ...
        if ([-4, 0, 4].some(ox => [-3, 0, 3].some(oy => p.field.sample(x + ox, y + oy).coverage < 0.999 || ['city', 'lake'].includes(p.field.regionAt(x + ox, y + oy) ?? '')))) continue;
        if (transport.some(c => vs2SegmentDistance({ x, y }, c.a, c.b) < c.radius + 4)) continue;
