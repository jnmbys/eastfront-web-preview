...
function renderDeploymentZone(model:BrowserRenderModel):string{
  ...
  return `<g id="deployment-zone-layer">${model.deployment.zoneKeys.map((key)=>{const hex=hexes.get(key);if(!hex)return '';return `<polygon data-role="deployment-hex" data-hex="${key}" points="${polygonPointsString(hex.coord)}" class="deployment-zone" role="button" tabindex="0" aria-label="${t('map.deploymentHex',{hex:key})}"/>`;}).join('')}</g>`;
}
...
export function renderCounterHit(counter:CounterModel):string {
  const hit=deriveTouchHitArea(counter);return `<rect data-hit-unit-id="${esc(counter.id)}" x="${hit.center.x-hit.side/2}" y="${hit.center.y-hit.side/2}" width="${hit.side}" height="${hit.side}" class="unit-hit-area"/>`;
}
...
export function coreSvgOverlayMarkup(model:BrowserRenderModel,options:CoreSvgOptions):string{
