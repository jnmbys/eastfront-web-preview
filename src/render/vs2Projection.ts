import { hexToPixel, hexPolygon, hexNeighbors, sharedHexEdge, type Point } from '../geometry/hex.js';
...
export interface VS2Corridor { readonly key: string; readonly kind: 'road' | 'rail' | 'river' | 'bridge'; readonly a: Point; readonly b: Point; readonly radius: number; }
export function vs2SegmentDistance(p: Point, a: Point, b: Point): number {
...
export function vs2RectangleSegmentDistance(p: Point, halfWidth: number, halfHeight: number, a: Point, b: Point): number {
  let lo = 0, hi = 1;
  ...
  if (lo <= hi) return 0;
  const pointRect = (q: Point) => Math.hypot(Math.max(Math.abs(q.x - p.x) - halfWidth, 0), Math.max(Math.abs(q.y - p.y) - halfHeight, 0));
  let distance = Math.min(pointRect(a), pointRect(b));
  for (const dx of [-halfWidth, halfWidth]) for (const dy of [-halfHeight, halfHeight]) distance = Math.min(distance, vs2SegmentDistance({ x: p.x + dx, y: p.y + dy }, a, b));
  return distance;
}

/** Projection only: canonical coordinates and topology remain the sole source. */
export function projectVS2Terrain(model: BrowserRenderModel, pixelSize = 2) {
  if (!Number.isFinite(pixelSize) || pixelSize <= 0) throw new Error('Invalid VS2 projection resolution');
  ...
  const cityBoundary: (readonly [Point, Point])[] = [];
  for (const cell of cityCells) for (const n of hexNeighbors(cell.coord)) {
    if (!cityKeys.has(`${n.q},${n.r}`)) cityBoundary.push(sharedHexEdge(cell.coord, n)!);
  }
  const corridors: VS2Corridor[] = [];
  for (const edge of [...model.edges].sort((a, b) => a.key.localeCompare(b.key))) {
    ...
    if (edge.road) corridors.push({ key: edge.key, kind: 'road', a, b, radius: 6 });
    if (edge.railway?.present) corridors.push({ key: edge.key, kind: 'rail', a, b, radius: 6 });
    if (edge.river) {
      const bank = sharedHexEdge(edge.a, edge.b);
      if (!bank) throw new Error(`Non-adjacent canonical river: ${edge.key}`);
  ...
  return {
    ...
    worldToRaster: (p: Point) => ({ x: (p.x - minX) / pixelSize, y: (p.y - minY) / pixelSize }),
    rasterToWorld: (p: Point) => ({ x: minX + p.x * pixelSize, y: minY + p.y * pixelSize }),
    canPlaceCity(p: Point, width: number, height: number): boolean {
      if (field.regionAt(p.x, p.y) !== 'city') return false;
      if (cityBoundary.some(([a, b]) => vs2RectangleSegmentDistance(p, width / 2, height / 2, a, b) === 0)) return false;
      return !corridors.some(edge => vs2RectangleSegmentDistance(p, width / 2, height / 2, edge.a, edge.b) <= edge.radius + 1);
    },
  };
}
export type VS2TerrainProjection = ReturnType<typeof projectVS2Terrain>;
