...
export class DynamicMapRenderer {
  ...
  update(layer:SVGGElement,model:BrowserRenderModel,options:CoreSvgOptions):{full:boolean;units:number} {
    ...
    const old=new Map(previous.counters.map(c=>[c.id,c]));let units=0;
    const groups=new Map<string,typeof model.counters>();
    for(const c of model.counters){const key=`${c.hex.q},${c.hex.r}`,group=groups.get(key)??[];group.push(c);groups.set(key,group);}
    ...
    for(const group of groups.values()){
      ...
      for(const [index,c] of group.entries()){
        const before=old.get(c.id);if(before?.selected===c.selected&&before?.combatRole===c.combatRole)continue;
        const current=elements.get(c.id);if(!current)continue;
        ...
        fragment.innerHTML=renderCounter(c,index,group.length);const next=fragment.firstElementChild!;
        for(const name of ['class','aria-label','aria-pressed'])current.setAttribute(name,next.getAttribute(name)!);
        const title=current.querySelector('title');if(title)title.textContent=next.querySelector('title')?.textContent??'';
        for(const [selector,attributes]of [['.counter-face',['transform']],['.counter-body',['stroke','stroke-width']]] as const){
          const target=current.querySelector(selector),source=next.querySelector(selector);
          if(target&&source)for(const name of attributes)target.setAttribute(name,source.getAttribute(name)!);
        }
        units++;
      }
    }
