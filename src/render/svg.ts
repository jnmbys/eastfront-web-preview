import {
  assertAdjacentPath, hexKey, hexLine, hexPolygon, hexToPixel,
  polygonPointsString, type HexCoord, type Point,
} from '../geometry/hex.js';
import {
  attackTargetHexes, bridgeEdges, enemyZocHexes, mapHexes,
  railPath, reachableHexes, riverEdges, roadPath, terrainAt, units, type Terrain, type Unit,
} from '../model/prototypeData.js';
import {
  SELECTED_VISUAL_SCALE, deriveBridgeGeometry, deriveCenterSegment, deriveCounterBounds,
  deriveCounterPlacement, deriveOverlayPolygon, deriveRiverEdge, deriveTargetBrackets, deriveTouchHitArea,
} from './derive.js';
...
function line(a: Point, b: Point, className: string, extra = ''): string {
...
function forestDetail(hex: HexCoord): string {
...
function terrainDetail(hex: HexCoord, terrain: Terrain): string {
  const c = hexToPixel(hex);
  if (terrain === 'forest') return forestDetail(hex);
  if (terrain === 'city') {
...
function pathSegments(path: readonly HexCoord[], className: string): string {
...
function renderBridge(a: HexCoord, b: HexCoord): string {
...
function renderBaseOverlays(): string {
...
function renderPath(path: readonly HexCoord[]): string {
...
function unitSymbol(unit: Unit): string {
  if (unit.kind === 'infantry') return `<path d="M-10 -9 L10 9 M10 -9 L-10 9"/>`;
  if (unit.kind === 'armor' || unit.kind === 'tank') return `<ellipse cx="0" cy="0" rx="11" ry="6.5"/>`;
  if (unit.kind === 'artillery') return `<circle cx="0" cy="0" r="5.5" fill="currentColor"/><path d="M-13 0 H13"/>`;
  return `<text x="0" y="4" text-anchor="middle" class="at-label">AT</text>`;
}

function factionMark(unit: Unit, side: number): string {
...
function renderCounterVisual(unit: Unit, selected: boolean, stackIndex: number, stackSize: number): string {
...
function groupedUnits(): UnitGroupInfo[] {
  const grouped=new Map<string,Unit[]>();
  for(const unit of units){const key=hexKey(unit.hex); const list=grouped.get(key)??[]; list.push(unit); grouped.set(key,list);}
  return [...grouped.entries()].map(([key,group])=>({key,group}));
}

function renderUnits(state: InteractionState): string {
...
function renderDebugInfrastructure(): string {
  return [
...
function renderDebugUnits(): string {
...
function renderDebug(path: readonly HexCoord[]): string {
...
export function makePath(selectedUnitId: string, destination: HexCoord): HexCoord[] {
...
function computeMapViewBox(): ViewBoxSpec {
...
export const MAP_VIEWBOX: Readonly<ViewBoxSpec> = Object.freeze(computeMapViewBox());

export function svgMarkup(state: InteractionState): string {
  const v=MAP_VIEWBOX;
