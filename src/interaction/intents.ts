...
function issuesMessage(issues:readonly {code:string;message:string}[]):string {
...
export function deploySelectedUnit(session:LocalGameSession,presentation:PresentationState,hex:HexCoord):void {
  ...
  if(!id){presentation.message='Select a deployment unit first.';return;}
...
export function extendMoveDraft(session:LocalGameSession,presentation:PresentationState,destination:HexCoord):void {
  ...
  if(!id){presentation.message='Select a controlled unit first.';return;}
  const unit=session.state.units[id];
  if(!unit){presentation.message='Selected unit is unavailable.';return;}
  ...
  if(presentation.pathDraft.length>0){
    ...
    if(sameHex(destination,previous)){presentation.pathDraft.pop();presentation.message='Removed last path step.';return;}
  }
  if(!adjacent(tail,destination)){presentation.message='Path draft can only extend to an adjacent Hex.';return;}
...
export function cancelMoveDraft(presentation:PresentationState):void {presentation.pathDraft=[];presentation.message='Movement draft cancelled.';}

export function commitMoveDraft(session:LocalGameSession,presentation:PresentationState):void {
  const id=presentation.selectedUnitId;
  if(!id||presentation.pathDraft.length===0){presentation.message='Draft at least one movement step first.';return;}
...
export function enterRailRepairMode(presentation:PresentationState):void {presentation.interactionMode='RAIL_REPAIR';presentation.message='Tap railway edges to add/remove them from the repair plan.';}
export function toggleRailRepairEdge(session:LocalGameSession,presentation:PresentationState,edgeKey:string):void {
  ...
  if(!edge?.railway?.present){presentation.message='That edge is not railway.';return;}
...
export function cancelRailRepair(presentation:PresentationState):void {presentation.railRepairEdgeKeys=[];presentation.selectedEngineerUnitId=null;presentation.message='Rail Repair plan cleared.';}
export function commitRailRepair(session:LocalGameSession,presentation:PresentationState):void {
  if(presentation.railRepairEdgeKeys.length===0){presentation.message='Select at least one railway edge.';return;}
...
export function deploySelectedReinforcement(session:LocalGameSession,presentation:PresentationState,entryHex:HexCoord):void {
  ...
  if(!id){presentation.message='Select a Soviet reinforcement first.';return;}
...
export function recoverSelectedUnit(session:LocalGameSession,presentation:PresentationState):void {
  const id=presentation.selectedUnitId;
  if(!id){presentation.message='Select a damaged controlled unit first.';return;}
...
function combatOutcomeMessage(action:string,accepted:boolean,issues:readonly {code:string;message:string}[]):string {
...
export function toggleAttackUnit(session:LocalGameSession,presentation:PresentationState,unitId:EntityId):void {
  if(session.state.pendingDecision){presentation.message='Resolve the current combat decision before declaring another attack.';return;}
  const unit=session.state.units[unitId];if(!unit||!unit.alive){presentation.message='Unit unavailable.';return;}
...
export function clearAttackDraft(presentation:PresentationState):void {presentation.attackUnitIds=[];presentation.attackTarget=null;presentation.attackerArtilleryUnitId=null;presentation.message='Attack draft cleared.';}
export function declareAttack(session:LocalGameSession,presentation:PresentationState):void {
  if(!presentation.attackTarget||presentation.attackUnitIds.length===0){presentation.message='Choose attacker(s) and an enemy target Hex first.';return;}
...
export function passCombatReaction(session:LocalGameSession,presentation:PresentationState):void {
  const pending=session.state.pendingDecision;if(pending?.kind!=='DEFENDER_REACTION'){presentation.message='No defender reaction is pending.';return;}
...
export function appendLossDraft(session:LocalGameSession,presentation:PresentationState,unitId:EntityId):void {
  const pending=session.state.pendingDecision;if(pending?.kind!=='LOSS_ALLOCATION'||!pending.eligibleUnitIds.includes(unitId)){presentation.message='Unit is not eligible for this loss allocation.';return;}
  presentation.lossDraft.push(unitId);presentation.interactionMode='LOSS_ALLOCATION';presentation.message=`Loss draft: ${presentation.lossDraft.length}/${pending.lossSteps} step(s).`;
}
export function undoLossDraft(presentation:PresentationState):void {presentation.lossDraft.pop();presentation.message='Removed last drafted loss.';}
export function clearLossDraft(presentation:PresentationState):void {presentation.lossDraft=[];presentation.message='Loss draft cleared.';}
export function commitLosses(session:LocalGameSession,presentation:PresentationState):void {
  const pending=session.state.pendingDecision;if(pending?.kind!=='LOSS_ALLOCATION'){presentation.message='No loss allocation is pending.';return;}
...
export function commitRetreat(session:LocalGameSession,presentation:PresentationState):void {
  const pending=session.state.pendingDecision;if(pending?.kind!=='RETREAT'){presentation.message='No retreat is pending.';return;}
