...
function patchShell(parent:Element,next:Element,locations:Element):void {
 const key=(el:Element,index:number)=>el.getAttribute('data-retained-deployment-locations')!==null?'class:deployment-locations':
 ...
 const kept=new Set<Element>();let cursor=parent.firstElementChild;
 for(const [index,wanted]of desired.entries()){
  const id=key(wanted,index),current=old.get(id);let node:Element;
  if(wanted.getAttribute('data-retained-deployment-locations')!==null)node=locations;
  else if(current&&(current.contains(locations)||current.getAttribute('class')==='roster-list')){patchShell(current,wanted,locations);node=current;}
  else if(current&&current.innerHTML===wanted.innerHTML&&current.textContent===wanted.textContent&&
   JSON.stringify(Array.from(current.attributes).map(a=>[a.name,a.value]))===JSON.stringify(Array.from(wanted.attributes).map(a=>[a.name,a.value])))node=current;
  else node=wanted;
  if(node===cursor)cursor=cursor.nextElementSibling;
  else if(current===cursor){cursor=current.nextElementSibling;current.replaceWith(node);}
  else parent.insertBefore(node,cursor);
  kept.add(node);old.delete(id);
 }
 for(const node of Array.from(parent.children))if(!kept.has(node))node.remove();
}
...
export class DeploymentPanelRenderer {
 ...
 private counts=new Map<string,number>();private chosen:string|null=null;private cardType='';
 ...
 update(panel:HTMLElement,owner:object,model:BrowserRenderModel,selected:string|null,ui:DeploymentTouch,markup:(locations?:string)=>string,beforeReplace?:()=>void):boolean {
  ...
  if(reuse){
   ...
   if(this.cardType!==row.type)for(const card of this.cards.values())card.children[0]!.textContent=t('deployment.unitTitle',{unit:enumLabel(row.type)});
   ...
   beforeReplace?.();this.clear();panel.innerHTML=markup();this.locations=panel.querySelector('.deployment-locations');
  ...
  this.owner=owner;this.panel=panel;this.key=key;this.counts=counts;this.chosen=chosen;this.cardType=row.type;return reuse;
 }
}
