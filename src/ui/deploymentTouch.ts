import { msg, type Message } from '../localization/index.js';
...
export interface DeploymentTouch {unitId:string|null;key:string|null;status:'idle'|'selected'|'deployed'|'invalid';message:Message;}
...
export function chooseDeploymentTarget(ui:DeploymentTouch,model:BrowserRenderModel,unitId:string|null,key:string):boolean{
 ...
 ui.unitId=unitId;ui.key=key;ui.status='selected';ui.message=msg('feedback.deploymentSelected');return true;
}
export function confirmDeploymentTarget(ui:DeploymentTouch,session:LocalGameSession,presentation:PresentationState):void{
 if(!ui.key||!ui.unitId||ui.unitId!==presentation.selectedDeploymentUnitId){ui.status='invalid';ui.message=msg('feedback.deploymentSelectAgain');return;}
 ...
 ui.status=accepted?'deployed':'invalid';ui.message=accepted?msg('feedback.deployedNext',{id}):deploymentRejectionMessage(session.lastResult?.issues??[]);
