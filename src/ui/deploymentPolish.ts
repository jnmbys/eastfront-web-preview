...
export function deploymentRejection(issues:readonly {code:string}[]):string{
 const messages:Record<string,string>={STACKING_LIMIT:'Maximum units reached. Choose another position.',INVALID_DEPLOYMENT_HEX:'This position is outside your deployment area. Choose a highlighted position.',DEPLOYMENT_UNIT_UNAVAILABLE:'This unit is unavailable. Select another reserve unit.',WRONG_PHASE:'Deployment is no longer active.',WRONG_SIDE:'Wait for your side’s deployment phase.',INVALID_CONTROLLER:'Your side could not be identified. Return to the current player’s view.'};
