import { EPSILON, hexDistance, hexPolygon, hexToPixel, polygonPointsString, sharedHexEdge, type Point } from '../geometry/hex.js';
...
import {
  SELECTED_VISUAL_SCALE, deriveCounterBounds, deriveCounterPlacement, deriveOverlayPolygon,
  deriveRiverEdge, deriveTargetBrackets, deriveTouchHitArea,
} from '../render/derive.js';
import { MAP_VIEWBOX, makePath, svgMarkup } from '../render/svg.js';

export interface AssertionResult { name: string; pass: boolean; }
const near=(a:number,b:number)=>Math.abs(a-b)<EPSILON;
const samePoint=(a:Point,b:Point)=>near(a.x,b.x)&&near(a.y,b.y);
const routeHasEdge=(route:readonly {q:number;r:number}[],a:{q:number;r:number},b:{q:number;r:number})=>route.slice(1).some((h,i)=>(h.q===b.q&&h.r===b.r&&route[i]!.q===a.q&&route[i]!.r===a.r)||(h.q===a.q&&h.r===a.r&&route[i]!.q===b.q&&route[i]!.r===b.r));
...
export function runGeometryAssertions(): AssertionResult[] {
  ...
  check('counter anchor = hex center', units.every(u=>{
    const p=deriveCounterPlacement(u); const c=hexToPixel(u.hex); return samePoint(p.authoritativeAnchor,c);
  }));
  ...
  check('damage/OOS anchor unchanged', units.every(u=>{
    ...
    const damage={...u,damage:(u.damage===2?0:2) as 0|1|2};
  ...
  check('river = shared edge', riverEdges.every(({a,b})=>{
    const d=deriveRiverEdge(a,b); const s=sharedHexEdge(a,b); return !!s && samePoint(d[0],s[0]) && samePoint(d[1],s[1]);
  }));
  check('river segments continuous', riverEdges.slice(1).every((edge,i)=>{
    const prev=deriveRiverEdge(riverEdges[i]!.a,riverEdges[i]!.b); const cur=deriveRiverEdge(edge.a,edge.b);
  ...
  check('stack bounds stay inside owning hex', (()=>{
    ...
    for(const u of units){const key=`${u.hex.q},${u.hex.r}`;const list=groups.get(key)??[];groups.set(key,[...list,u]);}
    for(const group of groups.values()) for(let i=0;i<group.length;i+=1){
      const u=group[i]!; const b=deriveCounterBounds(u,i,group.length);
      const corners=[{x:b.x,y:b.y},{x:b.x+b.width,y:b.y},{x:b.x+b.width,y:b.y+b.height},{x:b.x,y:b.y+b.height}];
      if(corners.some(c=>!insideConvex(c,hexPolygon(u.hex)))) return false;
    }
    return true;
  })());
