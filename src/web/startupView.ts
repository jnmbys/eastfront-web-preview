...
import { StartupProgress, type StartupSnapshot } from './startupProgress.js';

function copy(progress: StartupSnapshot) {
  return {
...
export function startupLoadingMarkup(progress: StartupSnapshot = new StartupProgress().snapshot): string {
...
export function updateStartupLoading(root: HTMLElement, progress: StartupSnapshot): void {
  const view = root.querySelector<HTMLElement>('.startup-progress');
  if (!view) return;
  const text = copy(progress);
  view.dataset.startupStage = progress.stage;
  const track = view.querySelector<HTMLElement>('.startup-track');
  track?.setAttribute('aria-valuenow', String(progress.completedSteps));
  track?.setAttribute('aria-valuetext', text.steps);
  const fill = view.querySelector<HTMLElement>('.startup-fill');
  if (fill) fill.style.transform = `scaleX(${progress.completedSteps / progress.totalSteps})`;
