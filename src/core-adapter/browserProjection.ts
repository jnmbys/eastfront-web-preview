...
export interface CounterModel {
  ...
  hex: HexCoord;
  selected: boolean;
  combatRole?: 'primary' | 'selected' | 'eligible';
  entrenched: boolean;
}
...
export interface CombatUiModel {
  attackDraft:{attackerUnitIds:string[];primaryAttackerId:string|null;eligibleAttackerIds:string[];target:HexCoord|null;targetHexes:HexCoord[];artilleryUnitIds:string[];selectedArtilleryId:string|null;issues:ValidationIssue[];preview:CombatContext|null};
  battle:Pick<CombatTransaction,'battleId'|'stage'|'targetHex'|'resolution'|'context'>|null; pending:GameState['pendingDecision'];
...
export function deriveBrowserRenderModel(session:LocalGameSession,presentation:PresentationState):BrowserRenderModel {
  ...
  const playerView=sessionPlayerView(session);
  const readOnly=playerView.viewer!==viewer.side;
  const visibleUnits=playerView.units;
  const visibleIds=new Set(visibleUnits.map(u=>u.id));
  const counters:CounterModel[]=visibleUnits.map(unit=>({id:unit.id,side:unit.side,type:unit.type,step:unit.step,stats:{...unit.stats},supplyState:unit.supplyState,controllerId:'friendly' in unit?unit.friendly.controllerId:'',hex:{...unit.hex},selected:unit.id===presentation.selectedUnitId,entrenched:unit.entrenched}));
  ...
  if(isDeploymentPhase(session.state)&&!readOnly){
  ...
  if(!readOnly&&selectedCounter&&(session.state.phase==='GERMAN_MOVEMENT'||session.state.phase==='SOVIET_MOVEMENT')){
  ...
  if(!readOnly&&playerView.viewer==='GERMAN'&&session.state.phase==='GERMAN_SUPPLY_RAIL'){
  ...
  if(!readOnly&&playerView.viewer==='SOVIET'&&session.state.phase==='SOVIET_REINFORCEMENT_SUPPLY'){
  ...
  if(!readOnly&&playerView.viewer===session.state.activeSide&&(session.state.phase==='GERMAN_RECOVERY'||session.state.phase==='SOVIET_RECOVERY')){
  ...
  if(!readOnly&&playerView.viewer===session.state.activeSide&&(session.state.phase==='GERMAN_ENTRENCHMENT'||session.state.phase==='SOVIET_ENTRENCHMENT')){
    const selected=selectedCounter?session.state.units[selectedCounter.id]:null;
  ...
  if(!readOnly&&(session.state.phase==='GERMAN_COMBAT'||session.state.phase==='SOVIET_COMBAT')&&(!session.state.pendingDecision||playerView.pendingDecision)){
    const pending=playerView.pendingDecision;
    ...
    const attackTarget=presentation.attackTarget&&visibleUnits.some(u=>u.side!==viewer.side&&u.hex.q===presentation.attackTarget!.q&&u.hex.r===presentation.attackTarget!.r)?presentation.attackTarget:null;
    const attackerUnitIds=presentation.attackUnitIds.filter((id)=>visibleIds.has(id)&&session.state.units[id]?.controllerId===session.activeViewerControllerId);
    const draftPrimary=primaryAttackerId(presentation);const primary=draftPrimary&&attackerUnitIds.includes(draftPrimary)?draftPrimary:null;
    ...
    if(isCombatTargetSelection(session,presentation)&&attackTarget){
      for(const counter of counters){
        if(counter.id===primary){counter.combatRole='primary';counter.selected=true;}
        else if(attackerUnitIds.includes(counter.id))counter.combatRole='selected';
        else if(eligibleAttackerIds.includes(counter.id))counter.combatRole='eligible';
      }
    }
    const targetKeys=new Set<string>();
    for(const id of attackerUnitIds){const u=session.state.units[id];if(!u)continue;for(const h of getNeighbors(u.hex)){const defenders=visibleUnits.filter((d)=>d.side!==u.side&&d.hex.q===h.q&&d.hex.r===h.r);if(defenders.length)targetKeys.add(`${h.q},${h.r}`);}}
    ...
    // Allow-list battle output: no full transaction, hidden HQ/support IDs or unresolved enemy decisions.
    const publicContext=(context:CombatContext|null):CombatContext|null=>context?{...structuredClone(context),attackerArtilleryUnitId:context.attackerArtilleryUnitId&&visibleIds.has(context.attackerArtilleryUnitId)?context.attackerArtilleryUnitId:null,defenderArtilleryUnitId:context.defenderArtilleryUnitId&&visibleIds.has(context.defenderArtilleryUnitId)?context.defenderArtilleryUnitId:null}:null;
    const publicBattle=battle?{battleId:battle.battleId,stage:battle.stage,targetHex:{...battle.targetHex},resolution:structuredClone(battle.resolution),context:publicContext(battle.context)}:null;
  ...
  const scrubIssues=(value:unknown):void=>{
    ...
    if('code' in value&&'message' in value&&hiddenIds.some(id=>JSON.stringify(value).includes(id))){
      const issue=value as ValidationIssue;issue.message='';delete issue.unitId;delete issue.hex;delete issue.details;return;
    }
