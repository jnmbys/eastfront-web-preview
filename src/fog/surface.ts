import { hexToPixel, HEX_SIZE, type Point } from '../geometry/hex.js';
import { getNeighbors } from '../core-adapter/core.js';
import { SPOTTING, type PlayerViewState } from '../player-view/playerView.js';
import { viewBoxForHexes, type ViewBoxSpec } from '../render/coreSvg.js';
...
export interface FogPlan { key:string; maskKey:string; viewer:PlayerViewState['viewer']; bounds:ViewBoxSpec; map:Point[]; visible:Point[]; recon:Point[]; }
export interface FogRaster { width:number;height:number;rgba:Uint8ClampedArray; }
const hexKey=(h:{q:number;r:number})=>`${h.q},${h.r}`;
/** No session, enemy roster, authoritative state or animation facts are accepted. */
export function deriveFogPlan(view:PlayerViewState,selectedUnitId:string|null=null):FogPlan {
  ...
  if(selected){let frontier=[selected.hex];for(let n=0;n<=SPOTTING.reconContact;n++){const next=frontier.flatMap(getNeighbors);for(const h of frontier)if(keys.includes(hexKey(h)))reconKeys.add(hexKey(h));frontier=next.filter(h=>!reconKeys.has(hexKey(h)));}}
...
function hash(x:number,y:number):number {let n=Math.imul(x,374761393)^Math.imul(y,668265263)^0x51af31;n=Math.imul(n^(n>>>13),1274126177);return ((n^(n>>>16))>>>0)/4294967295;}
...
export function rasterizeFog(plan:FogPlan,kind:'fog'|'recon'='fog'):FogRaster {
  ...
  for(let y=0;y<height;y++)for(let x=0;x<width;x++){
    ...
    else {rgba[i]=Math.round(83+broad*10);rgba[i+1]=Math.round(101+broad*9);rgba[i+2]=Math.round(118+broad*7);rgba[i+3]=Math.round(255*coverage*amount*(FOG_STYLE.opacity+(fine-.5)*.025));}
  }
