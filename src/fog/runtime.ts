...
import {deriveFogPlan,rasterizeFog,FOG_STYLE,type FogPlan,type FogRaster} from './surface.js';
export type FogEncoder=(raster:FogRaster)=>string;
function encode(raster:FogRaster):string {
  const canvas=document.createElement('canvas');canvas.width=raster.width;canvas.height=raster.height;
  const ctx=canvas.getContext('2d');if(!ctx)throw new Error('Fog canvas unavailable');
  const data=ctx.createImageData(raster.width,raster.height);data.data.set(raster.rgba);ctx.putImageData(data,0,0);return canvas.toDataURL('image/png');
}
interface Cached {plan:FogPlan;fog:string;recon:string|null;}
...
export class FogRuntime {
  ...
  sync(svg:SVGSVGElement|null,view:PlayerViewState|null,selectedUnitId:string|null=null,instant=false):void {
    ...
    if(!entry){
      ...
      if(fog===undefined){fog=plan.viewer==='OBSERVER'?'':this.encoder(rasterizeFog(plan));if(plan.viewer!=='OBSERVER')this.builds++;this.masks.set(plan.maskKey,fog);while(this.masks.size>3)this.masks.delete(this.masks.keys().next().value!);}
      entry={plan,fog,recon:plan.recon.length?this.encoder(rasterizeFog(plan,'recon')):null};this.cache.set(plan.key,entry);while(this.cache.size>3)this.cache.delete(this.cache.keys().next().value!);
    }
    const node=(url:string,role:string)=>{const image=svg.ownerDocument.createElementNS('http://www.w3.org/2000/svg','image');const b=plan.bounds;for(const [key,value]of Object.entries({x:b.minX,y:b.minY,width:b.width,height:b.height,href:url,'data-fog-role':role,'preserveAspectRatio':'none'}))image.setAttribute(key,String(value));layer.appendChild(image);return image;};
    this.current=entry;layer.setAttribute('data-fog-viewer',plan.viewer);layer.setAttribute('data-fog-builds',String(this.builds));
    if(!entry.fog)return;
    ...
    const current=node(entry.fog,'current');if(entry.recon)node(entry.recon,'recon');
    if(old){
      ...
      current.addEventListener('animationend',done);this.cleanup=done;
    }
  }
}
