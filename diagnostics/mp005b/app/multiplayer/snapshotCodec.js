...
const str = v => typeof v === 'string' && v.length <= 4096;
const one = (...xs) => v => xs.includes(v);
const side = one('GERMAN', 'SOVIET'), nullable = (c) => v => v === null || c(v);
const list = (c, max = 4096) => v => Array.isArray(v) && v.length <= max && v.every(c);
const ids = list(id), coords = list(hex), strings = list(str), obj = record;
const required = (v, checks) => record(v) && Object.entries(checks).every(([k, c]) => Object.hasOwn(v, k) && c(v[k]));
const stats = v => shape(v, { attack: num, defense: num, movement: num });
...
const pending = v => {
    if (v === null)
        return true;
    if (!required(v, { battleId: id, side, decisionOwnerControllerId: id, eligibleControllerIds: ids }) || !record(v))
        return false;
    switch (v.kind) {
        ...
        case 'BREAKTHROUGH_OPTION':
        case 'SCHWERPUNKT_OPTION': return ids(v.eligibleUnitIds);
        default: return false;
    }
};
const victory = v => required(v, { winner: nullable(side), reason: nullable(str) });
const mapHex = v => shape(v, { coord: hex, terrain: one('PLAIN', 'FOREST', 'HILL', 'MARSH', 'ROUGH', 'CITY', 'MAIN_CITY', 'OUTER_CITY', 'LAKE'), control: nullable(side) }, { cityId: str });
const edge = v => shape(v, { key: str, a: hex, b: hex, road: bool, railway: nullable(v => shape(v, { present: bool, repairedBy: nullable(side), destroyed: bool })), river: nullable(one('MINOR', 'MAJOR')), bridge: nullable(v => shape(v, { kind: one('ROAD', 'RAILWAY', 'BOTH'), destroyed: bool })) });
function playerView(v) {
    if (!record(v) || !shape(v, { viewer: side, turn: revision, phase: str, activeSide: side, hexes: list(mapHex), edges: list(edge, 16384),
        ...
        identifiedHexKeys: strings, contactHexKeys: strings, resources: r => record(r) && Object.keys(r).every(k => k === v.viewer) && Object.values(r).every(x => shape(x, { rp: num, cp: num })), pendingDecision: pending, victory }))
        return false;
    return (v.pendingDecision === null || v.pendingDecision.side === v.viewer) &&
        v.units.every(u => !('friendly' in u) || (u.side === v.viewer && record(u.friendly) && u.friendly.side === v.viewer && u.friendly.id === u.id));
}
const counter = v => shape(v, { ...unitBase, controllerId: str, selected: bool }, { combatRole: one('primary', 'selected', 'eligible') });
const option = v => required(v, { hex, legal: bool, issues: list(obj), spentMP: num, maxMP: num });
const modelFields = { readOnly: bool, phase: str, turn: revision, activeSide: side, rp: obj, cp: obj, viewerControllerId: id, viewerSide: side, counters: list(counter), selectedCounter: nullable(counter),
...
const point = v => shape(v, { x: num, y: num });
const participant = v => shape(v, { unitId: id, position: point, offset: point, direction: point, character: one('generic', 'infantry', 'armor', 'artillery') });
const event = v => {
    if (!required(v, { id: str, actionId: str }) || !record(v))
        return false;
    if (['move', 'retreat', 'advance', 'breakthrough'].includes(String(v.kind)))
        return required(v, { unitId: id, path: coords, sourceOffset: point, destinationOffset: point });
    if (['hit', 'destroyed'].includes(String(v.kind)))
        return required(v, { unitId: id, position: point, participant });
...
function safeTree(v, depth = 0, budget = { remaining: 160000 }) {
...
export function decodeSnapshot(value, compactAllowed) {const __end=mp5.begin("decodeSnapshot",arguments[0]);try{
