import {trace as __perf002} from './trace.js';
import { NetworkPlayerSession } from '../../app/multiplayer/networkSession.js';
import { mt } from '../../app/multiplayer/catalog.js';
import { isNetwork, isSessionDeployment, sessionPlayerView, dispatchGameAction, deriveBrowserRenderModel } from '../../app/multiplayer/playerSession.js';
import { addMultiplayerHomeButton, mountLobby } from '../../app/multiplayer/lobby.js';
import { DynamicMapRenderer } from '../../app/render/dynamicMap.js';
import { DeploymentPanelRenderer } from '../../app/ui/deploymentPanelRenderer.js';
import { modelControls, bindModelControls } from '../../app/ui/modelControls.js';
import { FogRuntime } from '../../app/fog/runtime.js';
import { startupProgress } from '../../app/web/startupProgress.js';
import { observeTerrainLoad } from '../../app/render/terrainLoadProgress.js';
import { UnitAnimationRuntime } from '../../app/presentation/runtime.js';
import { animationControls, bindAnimationControls } from '../../app/ui/animationControls.js';
import { continueCombatFlow, chooseRetreatDestination, chooseRetreater, chooseAdvancer, chooseAdvanceDestination, routeCombatDecisionCounter, undoRetreatDestination } from '../../app/multiplayer/playerSession.js';
import { languageControl, bindLanguageControl } from '../../app/localization/languageControl.js';
import { issueText } from '../../app/localization/issues.js';
import { t, msg, enumLabel, phaseName, formatMessage } from '../../app/localization/index.js';
import { combatAttackPanel } from '../../app/ui/combatAttackPanel.js';
import { deploymentFocus, deploymentRejection } from '../../app/ui/deploymentPolish.js';
import { createDeploymentTouch, chooseDeploymentTarget, confirmDeploymentTarget } from '../../app/ui/deploymentTouch.js';
import { commandHeader, deploymentLocations, deploymentConfirm, deploymentFeedback, unitDescription, unitLabel } from '../../app/ui/commandPresentation.js';
import { coreHexKey, isDeploymentPhase } from '../../app/core-adapter/core.js';
import { createLocalGameSession, loadProductionMapFromUrl } from '../../app/core-adapter/session.js';
import { chooseLossAndContinue, cancelMoveDraft, cancelRailRepair, clearAttackDraft, clearLossDraft, commitBreakthrough, commitMoveDraft, commitRailRepair, commitSchwerpunkt, confirmPrivacyGate, attackAndContinue, deploySelectedReinforcement, deploySelectedUnit, enterRailRepairMode, entrenchSelectedUnit, extendBreakthroughDraft, extendMoveDraft, passAdvance, passBreakthrough, passCombatReaction, passSchwerpunkt, readyForPhase, recoverSelectedUnit, routeCombatTarget, isCombatTargetSelection, combatTargetIssues, selectAttackerArtillery, selectBreakthroughUnit, selectCounter, selectDeploymentRosterUnit, selectRailEngineer, selectReinforcement, selectSchwerpunktTarget, switchViewerForDevelopment, toggleAttackUnit, toggleSupportingAttacker, toggleRailRepairEdge, undoBreakthroughDraft, undoLossDraft, undoMoveDraft, useDefenderArtillery, } from '../../app/multiplayer/playerSession.js';
import {} from '../../app/render/coreModel.js';
import { coreSvgDynamicMarkup, coreSvgMarkup, viewBoxForHexes } from '../../app/render/coreSvg.js';
import { selectTerrainLod } from '../../app/render/terrainAssets.js';
import { buildCachedTerrainSurface, formatTerrainSurfaceFailure, terrainSurfaceCapabilities } from '../../app/render/terrainSurface.js';
import { HEX_SIZE } from '../../app/geometry/hex.js';
import { createPresentationState } from '../../app/state/presentation.js';
import { TERRAIN_VISUAL_SEED, createFreshProductionSession, defaultMapViewport, fatalMarkup, gameOverMarkup, homeMarkup, loadingMarkup, loadProductionRuntimeManifest, mobileAdvisoryMarkup, privacyHandoffMarkup, productionDeveloperUiAllowed, responsiveProfile, WEB_PREVIEW_VERSION } from '../../app/web/preview.js';
import { beginMapGesture, dragSuppressesTap, gesturePanViewport, updateMapGesture, zoomMapAt, pinchMapViewport } from '../../app/web/mapInteraction.js';
...
const __perfEnd=__perf002.begin("syncFogSurface");try{
...
const __perfEnd=__perf002.begin("startNewGame");try{
 deploymentTouch = createDeploymentTouch(); if (!productionMap || !cachedTerrainSurface) {
    appStatus = 'FATAL';
...
const __perfEnd=__perf002.begin("refreshDynamicView");try{
...
function render() {
const __perfEnd=__perf002.begin(`render:${appStatus}`);try{
...
function bind() {
const __perfEnd=__perf002.begin("bind");try{
...
const __perfEnd=__perf002.begin("bindDynamic");try{
...
const __perfEnd=__perf002.begin("enterNetworkMatch");try{
...
const __perfEnd=__perf002.begin("boot");try{
    ...
    try {
        const [map] = await Promise.all([__perf002.async("map-data",()=>loadProductionMapFromUrl()), __perf002.async("runtime-manifest",()=>loadProductionRuntimeManifest())]);
        productionMap = map;
        startupProgress.complete('resources');
        phase = 'static-terrain-surface';
        const terrainModel = __perf002.sync("initial-terrain-model",()=>networkModel ?? deriveBrowserRenderModel(createFreshProductionSession(map, TERRAIN_VISUAL_SEED), createPresentationState(false, false)));
        const vs2 = await __perf002.async("vs2-module",()=>loadVS2TerrainSurfaceHooks());
        startupProgress.complete('model');
        startupProgress.building();
        for (const lod of ['far', 'medium', 'close']) {
            cachedTerrainSurface = await __perf002.async(`surface:${lod}`,()=>buildCachedTerrainSurface(terrainModel, TERRAIN_VISUAL_SEED, 'p5', lod, vs2.worldBase));
...
export async function loadVS2TerrainSurfaceHooks() {
    const { createVS2TerrainSurfaceHooks } = await import('../../app/render/vs2TerrainSurface.js');
