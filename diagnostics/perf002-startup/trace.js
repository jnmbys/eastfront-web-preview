...
const spans=[],network=[],longTasks=[],frameGaps=[],clicks=[],assets=[];
...
function publish(){pendingExport=false;report.textContent=JSON.stringify({diagnosticOnly:true,userAgent:navigator.userAgent,viewport:{width:innerWidth,height:innerHeight,dpr:devicePixelRatio},clock:'performance.now; navigation-relative milliseconds',timeOrigin:performance.timeOrigin,spans,network,longTasks,frameGaps,clicks,assets,loadedImages,resources:resourceSummary(),boundaries:['requestAnimationFrame gaps are scheduling proxies, not presented pixels','No hidden payload, token, unit identity or GameState recorded','Resource durations can overlap; transferSize=0 may be cache or unreported','Image decode and GPU paint are not separately timed']});}
