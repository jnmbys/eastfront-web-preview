...
(()=>{
 ...
 let version='MP-005C',socket=null,lastConfirm=null;
 document.addEventListener('click',event=>{const button=event.target?.closest?.('#confirm-deployment');if(button&&!button.disabled){const at=now();lastConfirm=at;queueMicrotask(()=>{if(lastConfirm===at)lastConfirm=null;});}},true);
 window.addEventListener('eastfront-transport-timing',event=>{
  const t=event.detail;if(!t)return;const row=rows.at(-1);if(!row)return;
  if(t.outcome==='invalid-snapshot'){row.fallbackReason='snapshot-validation-failed; bounded full recovery';queueMicrotask(publish);return;}
  const meta=row.messages.findLast(m=>m.type===t.type&&m.sequence===t.sequence&&m.revision===t.revision);if(!meta)return;
  meta.production={callbackAt:t.callbackAt,parseAndEnvelopeMs:t.parsedAt-t.callbackAt,validateRebuildMs:t.decodedAt-t.parsedAt,receiveNotifyMs:t.appliedAt-t.decodedAt,processedAt:t.appliedAt};
  const status=document.querySelector('#network-match-status');meta.appliedRevision=Number(status?.dataset.revision);meta.interactive=status?.dataset.interactive==='true';
  if(t.type==='PLAYER_VIEW_SNAPSHOT'){const s=submissions.findLast(s=>s.acceptedRevision===t.revision&&s.snapshotSequence===t.sequence);if(s){s.production=meta.production;s.controlsObservedAt=now();s.appliedRevision=meta.appliedRevision;s.controlsReady=meta.interactive;}}queueMicrotask(publish);
 });
 ...
 function observe(ws){
  const row={url:ws.url,state:'CONNECTING',started:now(),requestedFormat:new URLSearchParams(location.search).get('snapshotFormat')==='snapshot-v1'?'snapshot-v1':null,negotiatedFormat:'snapshot-v1',messages:[]};rows.push(row);if(rows.length>5)rows.shift();
  const send=ws.send;ws.send=function(data){const at=now();try{const m=JSON.parse(String(data));if(m.messageType==='HELLO')row.helloSent=at;if(m.messageType==='SUBMIT_ACTION'){const s={requestId:m.requestId,expectedRevision:m.payload.expectedRevision,sentAt:at,confirmAt:lastConfirm};lastConfirm=null;pending.set(m.requestId,s);submissions.push(s);if(submissions.length>64){pending.delete(submissions.shift().requestId);}}if(m.messageType==='SET_SNAPSHOT_FORMAT'){row.negotiationRequestId=m.requestId;row.requestedFormat=m.payload.format;}if(m.messageType==='RESYNC_MATCH')row.fallbackReason='full-resync-requested';if(['QUERY_MATCH','RESYNC_MATCH'].includes(m.messageType))row[m.messageType]=(row[m.messageType]??0)+1;}catch{}return send.call(this,data);};
  ...
  ws.addEventListener('message',event=>{
   ...
   if(m.messageType==='SNAPSHOT_FORMAT_SELECTED'&&m.requestId===row.negotiationRequestId){row.negotiatedFormat=m.payload.format;row.fallbackReason=null;publish();return;}
   if(m.messageType==='ROOM_ERROR'&&m.requestId===row.negotiationRequestId){row.negotiationError=m.payload.code;if(m.payload.code==='UNSUPPORTED_MESSAGE'){row.negotiatedFormat='snapshot-v1';row.fallbackReason='old-server-unsupported-capability';}publish();return;}
