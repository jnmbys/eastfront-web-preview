...
const before=execFileSync('git',['show','ddddcbb65057946436d0efcba1025615f89d353d:styles.css'],{encoding:'utf8'}),after=readFileSync('styles.css','utf8');
const old=execFileSync('git',['show','ddddcbb65057946436d0efcba1025615f89d353d:src/main.ts'],{encoding:'utf8'}),now=readFileSync('src/main.ts','utf8');
...
let html='<!doctype html><meta charset="utf-8"><title>EASTFRONT · UI Visual Pass A</title><style>body{margin:24px;background:#101722;color:#eee5d4;font:16px system-ui}p{max-width:950px;line-height:1.6;color:#bac6d5}iframe{width:100%;height:800px;border:1px solid #7c6b50;margin-bottom:30px}h2{font:28px Georgia}a{color:#d8c39a}</style><h1>UI Visual Pass A · Before / After</h1><p>Static HTML comparison, not browser screenshots. Home uses production markup; deployment uses the actual production roster function and fresh scenario projection. HUD is a representative component arrangement with the unchanged D map export. Buttons have visual states but no game actions. Resize the window to inspect responsive CSS. This is not a live gameplay or tablet acceptance test.</p>';
for(const [name,css,source] of [['Before',before,old],['After',after,now]])html+=`<h2>${name} · Main menu</h2>`+frame(css,homeMarkup('DESKTOP'))+`<h2>${name} · HUD / Deployment / Controls</h2>`+frame(css,hud(roster(source)));
writeFileSync('evidence/ui-visual-pass-a/BEFORE-AFTER.html',html);
