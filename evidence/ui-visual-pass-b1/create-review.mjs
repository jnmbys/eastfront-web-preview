...
import {deploymentLocations,deploymentFeedback,unitDescription,unitLabel,escapeUi,commandHeader} from '../../dist/app/ui/commandPresentation.js';
...
const panel=Function('model','presentation','deploymentTouch','esc','sideLabel','deploymentLocations','deploymentFeedback','unitDescription','unitLabel',js+';return deploymentPanel(model);')(model,presentation,deploymentTouch,escapeUi,s=>s==='SOVIET'?'Soviet':'German',deploymentLocations,deploymentFeedback,unitDescription,unitLabel);
const map='data:image/png;base64,'+readFileSync('evidence/vs2-002r-e/01-full-vs2-far.png').toString('base64');
const html=`<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>EASTFRONT B1 static deployment review</title><style>${readFileSync('styles.css','utf8')}</style><header class="topbar"><div class="brand"><strong>EASTFRONT</strong></div><div class="turn-strip command-hud">${commandHeader(model)}</div><div class="resource-strip"><span>STATIC REVIEW</span></div></header><main class="workspace panel-open"><section class="map-card"><div class="map-toolbar"><strong>B1 Deployment · static fixture</strong></div><div class="map-wrap"><img alt="Unchanged D terrain evidence" src="${map}" style="width:100%;height:100%;object-fit:contain"></div></section><aside class="side-panel">${panel}</aside></main><footer>Static HTML. Buttons do not perform game actions. Not a browser or device acceptance capture.</footer>`;
writeFileSync('evidence/ui-visual-pass-b1/DEPLOYMENT.html',html);
